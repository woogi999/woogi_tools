...
for (const site of [
  ...OURS,
  ...fromSherlock(await getJson(SHERLOCK)),
  ...fromWhatsMyName(await getJson(WHATSMYNAME)),
  ...fromMaigret(await getJson(MAIGRET)),
]) {
...
// Two sources can name the same site differently; keep names unique.
const names = new Set();
const sites = kept
...
export const USERNAME_SITES = ${JSON.stringify(sites)};
`,
);
console.log(
