...
async function open(file, { password = '' } = {}) {
  ...
  const run = (args) => {
    try {
      ...
      if (error?.status)
        throw new Error(
          '7-Zip couldn’t read this archive; it may be damaged, or need a password.',
        );
    }
  };
...
export async function listArchive(file, options = {}) {
  ...
  const walk = (dir, prefix) => {
    for (const name of z.FS.readdir(dir)) {
      ...
      if (stat.mode & 16384) walk(full, `${prefix}${name}/`);
        ...
        entries.push({ path: `${prefix}${name}`, name, size: stat.size, full });
    }
  };
  walk('/out', '');
  ...
  return {
    ...
    read: (entry) => new Uint8Array(z.FS.readFile(entry.full)),
  };
}
...
export const typeOf = (name) =>
  TYPES[extOf(name)] ?? 'application/octet-stream';
