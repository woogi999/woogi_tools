...
function detect(ua, list) {
  for (const item of list) {
    if (item.exact) {
      if (item.guard ? item.guard(ua) : item.re.test(ua))
        return { name: item.name, version: null };
      continue;
    }
    ...
    if (match)
      return {
        name: item.name,
        version: item.format ? item.format(match[1]) : match[1],
      };
  }
  return null;
}
...
export function parseUserAgent(ua) {
  ...
  if (os?.name === 'Windows' && WINDOWS_NAMES[os.version])
    os = { name: WINDOWS_NAMES[os.version], version: os.version };
  ...
  const bits = /WOW64|Win64|x64|x86_64|arm64|aarch64/i.test(ua)
    ? '64-bit'
    : /Windows NT|Intel|Linux/i.test(ua)
      ? '32-bit (or unspecified)'
      : null;
