import { loadFFmpeg, loadMagick, loadPandoc, loadSevenZip, loadPdfJs, loadJsPdf } from './engines';
...
const blobOf = (data, ext) => new Blob([data], { type: FORMATS.get(ext)?.mime ?? 'application/octet-stream' });
...
async function drawToCanvas(blob, { maxSize = Infinity, background = null } = {}) {
...
const canvasHandler = {
  ...
  pairs: [[['png', 'jpg', 'webp', 'bmp', 'avif', 'svg', 'ico'], ['png', 'jpg', 'webp', 'bmp']]],
  async convert(blob, from, to) {
    const opaque = to === 'jpg' || to === 'bmp';
    const canvas = await drawToCanvas(blob, { background: opaque ? '#fff' : null });
    if (to === 'bmp') return encodeBmp(canvas);
    return new Promise((resolve, reject) => canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('Encoding failed'))), FORMATS.get(to).mime, 0.92));
  },
};
...
const traceHandler = {
  ...
  async convert(blob) {
    ...
    const svg = ImageTracer.imagedataToSVG(imageData, { numberofcolors: 24, scale: 1, ltres: 1, qtres: 1, pathomit: 4 });
...
const MAGICK_READ = ['png', 'jpg', 'webp', 'gif', 'bmp', 'tiff', 'avif', 'heic', 'heif', 'jxl', 'jp2', 'ico', 'cur', 'apng', 'psd', 'psb', 'xcf', 'aseprite', 'tga', 'dds', 'exr', 'hdr', 'qoi', 'pcx', 'ppm', 'pgm', 'pbm', 'pnm', 'pam', 'sgi', 'sun', 'wbmp', 'xbm', 'xpm', 'jng', 'mng', 'dcx', 'pict', 'fits', 'farbfeld', 'miff', 'svg'];
const RAW_READ = ['dng', 'cr2', 'cr3', 'crw', 'nef', 'nrw', 'arw', 'sr2', 'srf', 'orf', 'rw2', 'raf', 'pef', 'erf', '3fr', 'kdc', 'mrw', 'x3f'];
const MAGICK_WRITE = ['png', 'jpg', 'webp', 'gif', 'bmp', 'tiff', 'avif', 'jxl', 'jp2', 'ico', 'cur', 'apng', 'psd', 'tga', 'dds', 'exr', 'hdr', 'qoi', 'pcx', 'ppm', 'pgm', 'pbm', 'pam', 'sgi', 'wbmp', 'xbm', 'xpm', 'fits', 'farbfeld', 'miff', 'pdf'];
...
const MULTI_FRAME = new Set(['gif', 'apng', 'webp', 'tiff', 'pdf', 'ico', 'mng', 'miff']);
  ...
  async convert(blob, from, to) {
    ...
    const data = ImageMagick.readCollection(bytes, settings, (images) => {
      ...
      if (to === 'ico' || to === 'cur') {
        ...
        if (image.width > ICON_MAX || image.height > ICON_MAX) image.resize(ICON_MAX, ICON_MAX);
...
const AUDIO_READ = ['mp3', 'wav', 'ogg', 'opus', 'flac', 'm4a', 'aac', 'wma', 'aiff', 'ac3', 'mka', 'au', 'caf', 'mp2', 'wv', 'ape', 'weba'];
const VIDEO_READ = ['mp4', 'webm', 'mkv', 'mov', 'avi', 'flv', 'wmv', 'mpeg', 'm4v', 'ogv', 'ts', '3gp', '3g2', 'mts', 'm2ts', 'vob', 'f4v', 'asf'];

const EVEN_SIZE = ['-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2'];
const H264 = [...EVEN_SIZE, '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '160k'];
...
const VIDEO_ARGS = {
  ...
  ts: H264,
  flv: H264,
  webm: ['-c:v', 'libvpx', '-b:v', '2M', '-deadline', 'realtime', '-cpu-used', '8', '-c:a', 'libvorbis'],
  ...
  gif: ['-vf', "fps=12,scale='min(480,iw)':-1:flags=lanczos,split[a][b];[a]palettegen[p];[b][p]paletteuse", '-an'],
};
...
async function runFFmpeg(blob, from, to, ctx) {
  ...
  const onProgress = ({ progress }) => ctx.progress(Math.max(0, Math.min(1, progress)));
  ffmpeg.on('log', onLog);
  ...
  try {
    ...
    if (code !== 0) {
      const reason = logs.reverse().find((line) => /error|invalid|not found|does not contain/i.test(line));
      throw new Error(reason ? `FFmpeg: ${reason.trim()}` : 'FFmpeg could not convert this file');
    }
...
const PANDOC_READ = { md: 'markdown', txt: 'markdown', html: 'html', docx: 'docx', odt: 'odt', rtf: 'rtf', epub: 'epub', tex: 'latex', typ: 'typst', rst: 'rst', org: 'org', textile: 'textile', wiki: 'mediawiki', dj: 'djot', ipynb: 'ipynb', dbk: 'docbook', opml: 'opml', fb2: 'fb2', csv: 'csv', tsv: 'tsv', xlsx: 'xlsx', pptx: 'pptx', bib: 'bibtex', ris: 'ris', adoc: 'asciidoc' };
const PANDOC_WRITE = { md: 'gfm', txt: 'plain', html: 'html', docx: 'docx', odt: 'odt', rtf: 'rtf', epub: 'epub', tex: 'latex', typ: 'typst', rst: 'rst', org: 'org', textile: 'textile', wiki: 'mediawiki', dj: 'djot', ipynb: 'ipynb', dbk: 'docbook5', opml: 'opml', fb2: 'fb2', pptx: 'pptx', adoc: 'asciidoc', texi: 'texinfo', icml: 'icml' };
const BINARY_DOCS = new Set(['docx', 'odt', 'epub', 'xlsx', 'pptx']);
const STANDALONE = new Set(['html', 'rtf', 'tex', 'typ', 'dbk', 'icml', 'texi', 'opml', 'fb2']);
...
const pandocHandler = {
  ...
  async convert(blob, from, to) {
    ...
    const files = { [inputName]: BINARY_DOCS.has(from) ? blob : await blob.text() };
    const options = { from: PANDOC_READ[from], to: PANDOC_WRITE[to], 'input-files': [inputName], 'output-file': outputName, standalone: STANDALONE.has(to) };
    const result = await pandoc.convert(options, null, files);
    const out = result.files?.[outputName];
    if (out === undefined) throw new Error(result.stderr?.trim() || 'Pandoc could not convert this document');
    return typeof out === 'string' ? blobOf(out, to) : new Blob([out], { type: FORMATS.get(to).mime });
  },
};

// ─── PDF.js (PDF → images / text) ───────────────────────────────────────

const pdfHandler = {
  id: 'pdf',
  engine: 'pdf',
  pairs: [[['pdf'], ['png', 'jpg', 'txt']]],
  async convert(blob, from, to, ctx) {
    ...
    try {
      ...
      for (let n = 1; n <= pdf.numPages; n++) {
        ...
        images.push(await new Promise((resolve) => canvas.toBlob(resolve, FORMATS.get(to).mime, 0.92)));
        ctx.progress(n / pdf.numPages);
      }
      if (images.length === 1) return images[0];
      const { zipSync } = await import('fflate');
      const entries = {};
      for (const [i, image] of images.entries()) entries[`${ctx.baseName}-page-${String(i + 1).padStart(3, '0')}.${to}`] = await bytesOf(image);
      return { blob: new Blob([zipSync(entries, { level: 0 })], { type: 'application/zip' }), ext: 'zip' };
    } finally {
      task.destroy();
    }
  },
...
const textPdfHandler = {
  ...
  async convert(blob) {
    ...
    const lines = doc.splitTextToSize((await blob.text()).replaceAll('\t', '    '), doc.internal.pageSize.getWidth() - margin * 2);
    let y = margin;
...
async function parseData(text, from) {
  ...
  const { data, errors } = Papa.parse(text.trim(), { header: true, dynamicTyping: true, skipEmptyLines: true, delimiter: from === 'tsv' ? '\t' : '' });
  if (!data.length && errors.length) throw new Error(errors[0].message);
  return data;
}

function toRows(value) {
  const rows = Array.isArray(value) ? value : Object.values(value ?? {}).find(Array.isArray) ?? [value];
  // Nested objects don't fit in a cell, so they're written as JSON text.
  return rows.map((row) => (row && typeof row === 'object' ? Object.fromEntries(Object.entries(row).map(([k, v]) => [k, v && typeof v === 'object' ? JSON.stringify(v) : v])) : { value: row }));
}
...
async function stringifyData(value, to) {
  ...
  if (to === 'toml') {
    ...
    const table = value && typeof value === 'object' && !Array.isArray(value) ? value : { items: value };
...
const dataHandler = {
  ...
  async convert(blob, from, to) {
    ...
    try {
      ...
      throw new Error(`Couldn't read this ${from.toUpperCase()} file: ${error.message}`);
    }
...
const ARCHIVE_READ = ['zip', '7z', 'tar', 'tar.gz', 'tar.bz2', 'tar.xz', 'gz', 'bz2', 'xz', 'rar', 'iso', 'cab', 'lzma', 'wim', 'zst'];
  ...
  async convert(blob, from, to) {
    ...
    const run = (args) => {
      try {
        ...
        if (error?.status) throw new Error('7-Zip could not process this archive');
      }
    };
    ...
    if (from.startsWith('tar.') && top.length === 1 && top[0].endsWith('.tar')) {
      z.FS.mkdir('/inner');
...
function sfntFlavorMatches(flavor, to) {
  ...
  if (to === 'otf' && !isCff) throw new Error('This font has TrueType outlines; convert it to TTF instead');
  if (to === 'ttf' && isCff) throw new Error('This font has CFF (OpenType) outlines; convert it to OTF instead');
}
...
async function sfntToWoff(bytes) {
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    tables.push({ tag: sfnt.getUint32(rec), checksum: sfnt.getUint32(rec + 4), length, data: compressed.length < length ? compressed : original });
    totalSfntSize += pad4(length);
  }
  const headerSize = 44 + 20 * numTables;
  const size = headerSize + tables.reduce((sum, t) => sum + pad4(t.data.length), 0);
...
async function woffToSfnt(bytes, to) {
  ...
  if (woff.getUint32(0) !== WOFF_SIGNATURE) throw new Error("This isn't a valid WOFF font");
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    tables.push({ tag: woff.getUint32(rec), checksum: woff.getUint32(rec + 16), data: compLength < origLength ? unzlibSync(raw) : raw });
  }
  const entrySelector = Math.floor(Math.log2(numTables));
  const searchRange = 2 ** entrySelector * 16;
  const headerSize = 12 + 16 * numTables;
  const out = new Uint8Array(headerSize + tables.reduce((sum, t) => sum + pad4(t.data.length), 0));
  const view = new DataView(out.buffer);
...
const fontHandler = {
  ...
  async convert(blob, from, to) {
    ...
    else if (to === 'woff2') out = await (await import('woff2-encoder')).compress(bytes);
    else if (from === 'woff') out = await woffToSfnt(bytes, to);
    else {
      out = await (await import('woff2-encoder/decompress')).default(bytes);
      sfntFlavorMatches(new DataView(out.buffer, out.byteOffset).getUint32(0), to);
    }
...
export const HANDLERS = [canvasHandler, traceHandler, fontHandler, dataHandler, pdfHandler, textPdfHandler, archiveHandler, magickHandler, ffmpegHandler, pandocHandler];
