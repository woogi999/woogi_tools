...
export async function collectDrop(dataTransfer) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory)
      folders.push({ name: entry.name, files: await readDirectory(entry, '') });
...
async function readDirectory(entry, prefix) {
  ...
  for (const child of await readEntries(entry.createReader())) {
    ...
    if (child.isDirectory)
      out.push(...(await readDirectory(child, `${prefix}${child.name}/`)));
    else
      out.push({ path: `${prefix}${child.name}`, file: await fileOf(child) });
  }
  return out;
}
...
export async function zipFolder({ name, files }, { onProgress } = {}) {
  ...
  const data = await new Promise((resolve, reject) => {
    zip(entries, { level: 6 }, (error, out) =>
      error ? reject(error) : resolve(out),
    );
  });
  onProgress?.(1);
  return new File([data], `${name || 'folder'}.zip`, {
