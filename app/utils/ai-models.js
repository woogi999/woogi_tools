...
const MODELS = {
  ...
  'whisper-tiny': { task: 'automatic-speech-recognition', id: 'onnx-community/whisper-tiny.en', label: 'Fast (~40 MB)' },
  'whisper-base': { task: 'automatic-speech-recognition', id: 'onnx-community/whisper-base', label: 'Better, any language (~80 MB)' },
  // A small instruction model, used for both rewriting and tidying up grammar.
  'flan-t5': { task: 'text2text-generation', id: 'Xenova/flan-t5-small', label: 'Writing model (~120 MB)' },
};
  ...
  const job = (async () => {
    ...
    return pipeline(spec.task, spec.id, {
      ...
      progress_callback: (event) => {
        if (event.status === 'progress' && event.total) onProgress?.({ file: event.file, ratio: event.loaded / event.total });
...
export async function ask(model, prompt, { max = 160 } = {}) {
  const out = await model(prompt, { max_new_tokens: max, temperature: 0.7, do_sample: true, top_p: 0.92, repetition_penalty: 1.15 });
