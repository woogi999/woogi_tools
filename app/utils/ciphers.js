...
export function railFence(text, rails, decode = false) {
  ...
  if (!decode) {
    let out = '';
    for (let rail = 0; rail < n; rail++) for (let i = 0; i < text.length; i++) if (pattern[i] === rail) out += text[i];
    return out;
  }
  const out = Array(text.length);
  let at = 0;
  for (let rail = 0; rail < n; rail++) for (let i = 0; i < text.length; i++) if (pattern[i] === rail) out[i] = text[at++];
  return out.join('');
}
...
export const toNumbers = (text) =>
  [...text.toUpperCase()]
    .map((ch) => (isLetter(ch.charCodeAt(0)) ? ch.charCodeAt(0) - A + 1 : ch === ' ' ? '/' : ch))
    .join(' ')
    .replaceAll(/\s+/g, ' ')
    .trim();
...
export const fromNumbers = (text) =>
  text
    .split(/[^0-9/]+/)
    .filter(Boolean)
    .map((part) => (part === '/' ? ' ' : String.fromCharCode(A + (Number(part) - 1))))
    .join('')
    .replaceAll(/\s+/g, ' ');
...
export const CIPHERS = [
  { id: 'caesar', label: 'Caesar shift', key: 'number', keyLabel: 'Shift', keyDefault: 3 },
  { id: 'rot13', label: 'ROT13' },
  { id: 'atbash', label: 'Atbash' },
  { id: 'vigenere', label: 'Vigenère', key: 'text', keyLabel: 'Keyword', keyDefault: 'key' },
  { id: 'railfence', label: 'Rail fence', key: 'number', keyLabel: 'Rails', keyDefault: 3 },
...
const ENGLISH = [8.2, 1.5, 2.8, 4.3, 12.7, 2.2, 2.0, 6.1, 7.0, 0.2, 0.8, 4.0, 2.4, 6.7, 7.5, 1.9, 0.1, 6.0, 6.3, 9.1, 2.8, 1.0, 2.4, 0.2, 2.0, 0.1];
const COMMON = ['the', 'and', 'that', 'have', 'for', 'not', 'with', 'you', 'this', 'but', 'his', 'from', 'they', 'she', 'her', 'been', 'are', 'was', 'were', 'what', 'when', 'there', 'their', 'would', 'about', 'hello', 'world'];
...
export function englishScore(text) {
  ...
  let difference = 0;
  for (let i = 0; i < 26; i++) difference += Math.abs((counts[i] / letters.length) * 100 - ENGLISH[i]);
...
export function crack(text, { limit = 8 } = {}) {
  ...
  const add = (cipher, note, out) => {
    ...
    if (clean.length > 1 && clean !== text.trim()) tries.push({ cipher, note, text: clean, score: englishScore(clean) });
  };
  for (let shift = 1; shift < 26; shift++) add('Caesar shift', `shift ${shift}`, caesar(text, -shift));
  add('Atbash', '', atbash(text));
  add('Reversed text', '', reverseText(text));
  if (/[.\-/]/.test(text) && /^[\s.\-/]+$/.test(text.trim())) add('Morse code', '', fromMorse(text));
  if (/\d/.test(text)) add('A1Z26', '', fromNumbers(text));
  for (let rails = 2; rails <= 10; rails++) add('Rail fence', `${rails} rails`, railFence(text, rails, true));
  ...
  for (let length = 2; length <= 6; length++) {
    ...
    if (key) add('Vigenère', `keyword "${key.toLowerCase()}"`, vigenere(text, key, true));
  }
