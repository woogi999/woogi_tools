...
export function parsePageRanges(text, pageCount) {
  ...
  if (!trimmed)
    return { groups: [Array.from({ length: pageCount }, (_, i) => i)] };
  const groups = [];
  for (const part of trimmed
    ...
    if (!match || (!match[1] && !match[3]))
      return { error: `"${part}" isn't a page or range like 2 or 4-7` };
    ...
    if (start > pageCount || end > pageCount)
      return {
        error: `This PDF only has ${pageCount} page${pageCount === 1 ? '' : 's'}`,
      };
