...
function readExif(view, start, tiffEnd) {
  ...
  const readIfd = (offset, names, prefix = '') => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      found.push({
        name: prefix + name,
        value: readValue(view, valueAt, type, length, little),
      });
    }
  };
...
function readValue(view, at, type, length, little) {
  if (type === 2) {
    let text = '';
    for (let i = 0; i < length && view.getUint8(at + i); i++)
  ...
  for (let i = 0; i < Math.min(length, 8); i++) {
    ...
    if (type === 1 || type === 7) numbers.push(view.getUint8(spot));
      ...
      numbers.push(
        view.getUint32(spot, little) / (view.getUint32(spot + 4, little) || 1),
      );
    else if (type === 9) numbers.push(view.getInt32(spot, little));
    else if (type === 10)
      numbers.push(
        view.getInt32(spot, little) / (view.getInt32(spot + 4, little) || 1),
      );
  }
  // Latitudes and longitudes come as degrees, minutes and seconds.
  if (type === 5 && numbers.length === 3)
    return `${numbers[0]}° ${numbers[1]}′ ${numbers[2].toFixed(2)}″`;
  return numbers
    .map((n) => (Number.isInteger(n) ? n : Number(n.toFixed(4))))
    .join(', ');
}
...
function pngText(bytes, chunk) {
  ...
  const key = new TextDecoder().decode(
    raw.subarray(0, split < 0 ? raw.length : split),
  );
  // Text chunks can hold nulls and other control codes; they would make a mess of the list.
  const decoded = new TextDecoder().decode(
    raw.subarray((split < 0 ? raw.length : split) + 1),
  );
  const value = [...decoded]
    .map((ch) => (ch.codePointAt(0) < 32 ? ' ' : ch))
    .join('');
...
export async function readMetadata(file) {
  ...
  if (isJpeg(bytes)) {
    ...
    for (const segment of jpegSegments(bytes)) {
      ...
      if (segment.marker === 0xe1) {
        const header = String.fromCharCode(
          ...bytes.subarray(segment.dataStart, segment.dataStart + 4),
        );
        if (header === 'Exif')
          fields.push(
            ...readExif(view, segment.dataStart + 6, segment.dataEnd),
          );
    ...
    if (extras)
      fields.push({
        name: 'Other blocks',
        value: `${extras} (XMP, Photoshop or comments)`,
      });
  ...
  if (isPng(bytes)) {
    ...
    for (const chunk of pngChunks(bytes)) {
      if (
        ...
        fields.push(pngText(bytes, chunk));
      else if (chunk.name === 'eXIf')
        fields.push(
          ...readExif(view, chunk.dataStart, chunk.dataStart + chunk.length),
        );
      else if (chunk.name === 'tIME')
        fields.push({ name: 'Changed on', value: 'yes' });
    }
