...
const KEYS = [
  { label: 'DEG', action: 'toggleAngle', kind: 'mode' },
  { label: '(', insert: '(' },
  { label: ')', insert: ')' },
  { label: '%', insert: '%' },
  ...
  { label: '⌫', action: 'backspace', kind: 'danger', aria: 'Backspace' },
...
export default class CalculatorPage extends Component {
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'calculator', ['expression', 'degrees']);
  }
  ...
  selection() {
    ...
    if (!el || document.activeElement !== el || el.value !== this.expression)
      return [end, end];
  ...
  backspace = () => {
    ...
    if (start === end && start > 0) {
      ...
      const fnMatch =
        /(asin|acos|atan|sinh|cosh|tanh|sin|cos|tan|log2|log|ln|sqrt|abs|Ans|√)\(?$/.exec(
          value.slice(0, start),
        );
  ...
  equals = () => {
    ...
    try {
      ...
      this.history = [
        { expression: text, result, degrees: this.degrees },
        ...this.history,
      ].slice(0, 50);
  ...
  <template>
    <ToolPage
      @route="calculator"
      @subtitle="Type or tap your maths. Understands stuff like 2π or 3(4+1), and Enter gives you the answer."
    >
      <div class="calc pop-in">
        <section class="math-card calc-main">
          <div class="calc-display {{if this.error 'has-error'}}">
            ...
            <span class="calc-preview">
              {{#if this.error}}{{this.error}}{{else if this.preview}}=
                {{this.preview}}{{/if}}
            </span>
          </div>
          <div class="calc-keys">
            {{#each this.keys as |key|}}
              <button
                type="button"
                class="calc-key {{if key.kind (concat 'is-' key.kind)}}"
                aria-label={{if key.aria key.aria key.label}}
                {{on "click" (fn this.press key)}}
              >
                {{#if (eq key.action "toggleAngle")}}{{if
                    this.degrees
                    "DEG"
                    "RAD"
                  }}{{else}}{{key.label}}{{/if}}
        ...
        <section class="math-card calc-history">
          <div class="field-head">
            <h3 class="qr-heading"><Icon @name="history" @size={{14}} />
              History</h3>
          ...
          {{/if}}
          <p class="tool-hint">Also understands sinh, cosh, tanh, log2, cbrt,
            exp, floor, ceil, round, mod(a,b), nroot(n,x), nCr(n,r), nPr(n,r),
            and |x| bars.</p>
        </section>
      </div>
    </ToolPage>
