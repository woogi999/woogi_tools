...
import {
  GITIGNORE_TEMPLATES,
  buildGitignore,
} from '../utils/gitignore-templates';
...
export default class GitignoreGeneratorPage extends Component {
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'gitignore-generator', ['selected']);
  }
  ...
  download = () => {
    const url = URL.createObjectURL(
      new Blob([this.output], { type: 'text/plain' }),
    );
  ...
  <template>
    <ToolPage
      ...
      @subtitle="Tick the languages, frameworks and editors you use and get one tidy .gitignore file."
    >
            ...
            {{#each this.templates key="id" as |t|}}
              <label
                class="settings-tool-item
                  {{unless (this.isSelected t.id) 'is-off'}}"
              >
                <input
                  type="checkbox"
                  checked={{this.isSelected t.id}}
                  {{on "change" (fn this.toggle t.id)}}
                />
