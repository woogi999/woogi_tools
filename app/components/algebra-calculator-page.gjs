...
export default class AlgebraCalculatorPage extends Component {
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'algebra-calculator', ['modeId']);
  }
  ...
  get output() {
    ...
    try {
      ...
      return result.lines
        ? result
        : {
            heading: this.mode.label === 'Factor' ? 'Factored' : 'Expanded',
            lines: [result.result],
            note: result.note,
            copy: result.result,
          };
  ...
  setInput = (event) =>
    (this.inputs = { ...this.inputs, [this.modeId]: event.target.value });
  useExample = (text) =>
    (this.inputs = { ...this.inputs, [this.modeId]: text });

  <template>
    <ToolPage
      @route="algebra-calculator"
      @subtitle="Solve equations, rearrange formulas, expand brackets and factor polynomials. Answers show up as you type."
    >
        ...
        <div class="math-tabs" role="tablist">
          {{#each this.modes as |m|}}
            <button
              type="button"
              role="tab"
              class="qr-tab {{if (eq this.modeId m.id) 'active'}}"
              aria-selected={{if (eq this.modeId m.id) "true" "false"}}
        ...
        <section class="math-card qr-mode-panel">
          <label class="math-field">
            <span class="qr-label is-muted">{{if
                (eq this.modeId "solve")
                "Equation"
                "Expression"
              }}</span>
            <textarea
              class="textarea algebra-input"
              rows="3"
              spellcheck="false"
              placeholder={{this.mode.placeholder}}
              value={{this.input}}
              {{on "input" this.setInput}}
            ></textarea>
          </label>
          <p class="tool-hint">{{this.mode.hint}}
            Use ^ for powers; 2x and 3(x+1) multiply.</p>
          <div class="notes-font-row">
            {{#each this.mode.examples as |example|}}
              <button
                type="button"
                class="qr-chip algebra-example"
