...
const KINDS = [
  { id: 'v4', label: 'UUID v4', hint: 'Fully random. The everyday choice.', make: () => crypto.randomUUID() },
  { id: 'v7', label: 'UUID v7', hint: 'Starts with a timestamp, so IDs sort in creation order. Great for database keys.', make: uuidV7 },
  { id: 'ulid', label: 'ULID', hint: '26 characters, time-sortable, no ambiguous letters.', make: ulid },
  { id: 'nano', label: 'Nano ID', hint: '21 URL-safe characters with about the same collision resistance as UUID v4.', make: () => nanoId() },
];
...
function describe(text) {
  if (/^[0-9A-HJKMNP-TV-Z]{26}$/i.test(text)) {
    const ms = [...text.slice(0, 10).toUpperCase()].reduce((n, c) => n * 32 + CROCKFORD.indexOf(c), 0);
    return [
      ['Type', 'ULID'],
  ...
  const variant = variantBits < 8 ? 'NCS (legacy)' : variantBits < 12 ? 'RFC 9562' : variantBits < 14 ? 'Microsoft (legacy)' : 'Reserved';
  const rows = [
    ['Type', `UUID version ${version}`],
    ['Variant', variant],
    ['Canonical', dashed(h)],
  ];
  if (version === 7) rows.push(['Created', new Date(parseInt(h.slice(0, 12), 16)).toLocaleString()]);
  if (version === 1) {
    // 60-bit count of 100ns intervals since 1582-10-15, split across three fields.
    const ticks = BigInt(`0x${h.slice(13, 16)}${h.slice(8, 12)}${h.slice(0, 8)}`);
    rows.push(['Created', new Date(Number(ticks / 10000n - 12219292800000n)).toLocaleString()]);
  }
  return rows;
}
        ...
        <section class="math-card">
          <div class="math-tabs" role="group" aria-label="ID type">
            {{#each this.kinds as |k|}}
              <button type="button" class="qr-tab {{if (eq this.kind k.id) 'active'}}" {{on "click" (fn this.setKind k.id)}}>{{k.label}}</button>
            {{/each}}
          </div>
          <p class="tool-hint">{{this.current.hint}}</p>
          <div class="math-row is-aligned">
            <label class="math-field"><span class="qr-label is-muted">How many</span><input type="number" min="1" max="500" class="math-input" value={{this.count}} {{on "input" this.setCount}} /></label>
            <button type="button" class="btn active math-swap" {{on "click" this.generate}}><Icon @name="refresh-cw" @size={{13}} /> Generate</button>
          </div>
          {{#if this.isUuid}}
            <div class="settings-actions">
              <label class="math-check"><input type="checkbox" checked={{this.uppercase}} {{on "change" this.toggleUppercase}} /> Uppercase</label>
              <label class="math-check"><input type="checkbox" checked={{this.hyphens}} {{on "change" this.toggleHyphens}} /> Hyphens</label>
            </div>
          {{/if}}
          ...
          <textarea class="textarea text-area-tall is-mono" readonly spellcheck="false" aria-label="Generated IDs" value={{this.allText}}></textarea>
        </section>

        <section class="math-card">
          <h3 class="qr-heading">Decode an ID</h3>
          <input type="text" class="math-input" spellcheck="false" placeholder="Paste a UUID or ULID" aria-label="ID to decode" value={{this.inspectInput}} {{on "input" this.setInspect}} />
          {{#if this.inspection}}
            <div class="math-stats">
              {{#each this.inspection as |row|}}
                <div class="math-stat"><span>{{row.label}}</span><strong>{{row.value}}</strong></div>
              {{/each}}
            </div>
          {{else}}
            <p class="tool-hint">Shows the version, variant and, for time-based IDs, when it was created.</p>
          {{/if}}
        </section>
      </div>
    </ToolPage>
