...
import { readMetadata, stripImage, isStrippableImage, extOf } from '../utils/metadata';
...
const MEDIA = ['mp3', 'm4a', 'wav', 'flac', 'ogg', 'opus', 'mp4', 'mkv', 'mov', 'webm', 'avi', 'm4v'];
const TAG_FIELDS = [
...
export default class MetadataEditorPage extends Component {
  ...
  async addFiles(list) {
    ...
    for (const file of files) {
      ...
      const item = { id: nextId++, file, media, image: isStrippableImage(file.name), fields: [], url: null, name: null, size: null, status: '', error: null };
      ...
      if (item.image) {
        try {
          ...
          this.update(item.id, { error: 'Couldn’t read what’s inside this one' });
        }
      }
    }
  }
  ...
  setTag = (key, event) => (this.tags = { ...this.tags, [key]: event.target.value });
  ...
  async handle(item, withTags) {
    ...
    try {
      if (item.image) {
        ...
        this.update(item.id, { url: URL.createObjectURL(blob), name: `${baseName(item.file.name)}-clean.${extOf(item.file.name)}`, size: blob.size, status: '', fields: [] });
        return;
      }
      if (item.media) {
        const ext = extOf(item.file.name);
        const tagArgs = withTags ? TAG_FIELDS.flatMap((f) => (this.tags[f.id] ? ['-metadata', `${f.id}=${this.tags[f.id]}`] : [])) : [];
        const blob = await runFFmpeg(item.file, {
          out: ext,
          type: item.file.type || '',
          build: (input) => ['-i', input, '-map_metadata', '-1', '-c', 'copy', ...tagArgs],
          onStatus: (status) => this.update(item.id, { status }),
        });
        this.update(item.id, { url: URL.createObjectURL(blob), name: `${baseName(item.file.name)}-clean.${ext}`, size: blob.size, status: '' });
        return;
      }
      this.update(item.id, { error: 'Nothing this tool can do with this sort of file yet', status: '' });
    } catch (error) {
      this.update(item.id, { error: error?.message ?? 'Couldn’t clean this one', status: '' });
    }
  }

  <template>
    <ToolPage @route="metadata-editor" @subtitle="See what's hidden inside your photos and media — camera, software, even where you were standing — and strip it out before you share.">
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          <label class="qr-drop fs-drop {{if this.dragging 'is-dragging'}}" {{on "dragover" this.dragOver}} {{on "dragleave" this.dragOver}} {{on "drop" this.drop}}>
            ...
            <span>{{if this.dragging "Drop them here" "Drop photos or media, paste them, or click to browse"}}</span>
            <input type="file" multiple class="sr-only" {{on "change" this.selectFiles}} />
          </label>
          <p class="tool-hint">JPEG and PNG are cleaned by cutting the metadata blocks straight out — the picture itself isn't touched or re-encoded, so nothing is lost. Audio and video are rewritten with their tags dropped, streams copied.</p>
          ...
          <div class="fs-frame fc-panel pop-in">
            <div class="fc-toolbar">
              ...
              <div class="settings-actions">
                <button type="button" class="btn active" disabled={{this.busy}} {{on "click" this.stripAll}}>{{if this.busy "Working…" "Strip it all out"}}</button>
                {{#if this.anyMedia}}
                  <button type="button" class="btn" disabled={{this.busy}} {{on "click" this.writeTags}}>Strip, then write my tags</button>
                {{/if}}
              ...
              <div class="math-row">
                {{#each this.tagFields as |f|}}
                  <label class="math-field">
                    ...
                    <input type="text" class="math-input" value={{get this.tags f.id}} {{on "input" (fn this.setTag f.id)}} />
                  </label>
                {{/each}}
              </div>
              <p class="tool-hint">These are written onto the audio and video files only, after the old tags are cleared. Leave them blank to just strip.</p>
            {{/if}}
                    ...
                    {{else if item.fields.length}}
                      <ul class="meta-list">
                        {{#each item.fields key="name" as |field|}}
                          <li class="meta-field"><span class="qr-label is-muted">{{field.name}}</span><span class="meta-value">{{field.value}}</span></li>
                        {{/each}}
                      </ul>
                    {{else if item.image}}
                      <span class="fs-row-size">Nothing hidden in here.</span>
                    {{else if item.media}}
                      <span class="fs-row-size">Audio or video: tags are cleared when you strip it.</span>
                    {{/if}}
                  </div>
                  <div class="fs-row-status">
                    {{#if item.url}}<a class="btn fs-save" href={{item.url}} download={{item.name}}><Icon @name="download" @size={{13}} /> Save clean copy</a>{{/if}}
                  </div>
