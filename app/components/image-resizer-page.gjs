...
async function decode(file) {
  try {
    ...
    throw new Error(
      "This browser can't open that image. Try the File Converter for HEIC, RAW and other formats.",
    );
  }
}
...
async function resize(file, options) {
  ...
  return {
    blob,
    name: `${base}-${width}x${height}.${ENCODABLE[type]}`,
    original,
    width,
    height,
  };
...
export default class ImageResizerPage extends Component {
  ...
  get pipWarning() {
    return 'Close the Image Resizer? The resize in progress will be cancelled.';
  }
  formats = FORMATS;
  ...
  setting = (key, event) => {
    ...
    this[key] = ['maxWidth', 'maxHeight', 'percent', 'quality'].includes(key)
        ...
        this.items = this.items.map((i) =>
          i.id === item.id
            ? {
                ...
                size: out.blob.size,
                name: out.name,
                dims: `${out.original.width}×${out.original.height} → ${out.width}×${out.height}`,
              }
            : i,
        );
      } catch (error) {
  ...
  <template>
    <ToolPage
      ...
      @subtitle="Drop in your images, pick a size and quality, and download them smaller. Works on lots at once."
    >
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          ...
          <div class="math-tabs" role="group" aria-label="Resize by">
            <button
              type="button"
              class="qr-tab {{if (eq this.mode 'fit') 'active'}}"
              {{on "click" (fn this.setMode "fit")}}
            >Fit within size</button>
            <button
              type="button"
              class="qr-tab {{if (eq this.mode 'percent') 'active'}}"
              {{on "click" (fn this.setMode "percent")}}
            >Percentage</button>
          </div>
          <div class="math-row">
            {{#if (eq this.mode "fit")}}
              <label class="math-field"><span class="qr-label is-muted">Max
                  width (px)</span><input
                  type="number"
                  min="0"
                  class="math-input"
                  value={{this.maxWidth}}
                  {{on "input" (fn this.setting "maxWidth")}}
                /></label>
              <label class="math-field"><span class="qr-label is-muted">Max
                  height (px)</span><input
                  type="number"
                  min="0"
                  class="math-input"
                  value={{this.maxHeight}}
                  {{on "input" (fn this.setting "maxHeight")}}
                /></label>
            {{else}}
              <label class="math-field"><span class="qr-label is-muted">Scale
                  (%)</span><input
                  type="number"
                  min="1"
                  max="400"
                  class="math-input"
                  value={{this.percent}}
                  {{on "input" (fn this.setting "percent")}}
                /></label>
            {{/if}}
          ...
          {{#if this.usesQuality}}
            <label class="math-field">
              ...
              <input
                ...
                max="100"
                value={{this.quality}}
                {{on "input" (fn this.setting "quality")}}
              />
            </label>
          {{/if}}
          <p class="tool-hint">{{if
              (eq this.mode "fit")
              "Images already smaller than the box keep their size, and proportions are always kept."
              "Scales width and height by the same amount."
            }}</p>
        </div>
            ...
            <div class="fc-toolbar">
              <div class="settings-actions">
                <button
                  ...
                  disabled={{this.busy}}
                  {{on "click" this.run}}
                >{{if this.busy "Working…" "Resize"}}</button>
                {{#if this.zipUrl}}<a
                    class="btn fs-save"
                    href={{this.zipUrl}}
