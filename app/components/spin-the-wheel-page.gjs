...
import { randomInt, shuffled } from '../utils/random';
...
export default class SpinTheWheelPage extends Component {
  @tracked text = 'Pizza\nSushi\nTacos\nRamen\nBurgers\nCurry';
  @tracked mode = 'wheel';
  @tracked rotation = 0;
  ...
  get slices() {
    ...
    return entries.map((label, i) => {
      ...
      const path =
        n === 1
          ? 'M100,100 m-96,0 a96,96 0 1,0 192,0 a96,96 0 1,0 -192,0'
          : `M100,100 L${100 + 96 * Math.cos(start)},${100 + 96 * Math.sin(start)} A96,96 0 ${large},1 ${100 + 96 * Math.cos(end)},${100 + 96 * Math.sin(end)} Z`;
  ...
  get wheelStyle() {
    ...
    return htmlSafe(
      `transform:rotate(${this.rotation}deg);transition:transform ${duration}ms cubic-bezier(0.12, 0.8, 0.2, 1)`,
    );
  }
  ...
  makeGroups = () => {
    ...
    if (this.groupMode === 'teams') {
      ...
      this.groups = teams.map((members, i) => ({
        title: `Team ${i + 1}`,
        members,
      }));
    } else {
      this.groups = [
        {
          title:
            size === 1 ? 'Winner' : `${Math.min(size, entries.length)} picks`,
          members: entries.slice(0, size),
        },
      ];
    }
  };
  ...
  setCount = (e) =>
    (this.count = Math.max(1, Math.floor(+e.target.value) || 1));
  toggleRemove = () => (this.removeWinner = !this.removeWinner);

  <template>
    <ToolPage
      @route="spin-the-wheel"
      @subtitle="Spin the wheel, pull a name out of the hat, or split a list into random teams. Let fate decide."
    >
      <div class="math-grid pop-in">
        <section class="math-card">
          <label class="field-label" for="rp-list">Entries ·
            {{this.entries.length}}</label>
          <textarea
            id="rp-list"
        ...
        <section class="math-card">
          <div class="math-tabs" role="group" aria-label="Mode">
            <button
              type="button"
              class="qr-tab {{if (eq this.mode 'wheel') 'active'}}"
              {{on "click" (fn this.setMode "wheel")}}
            >Wheel</button>
            <button
              type="button"
              class="qr-tab {{if (eq this.mode 'groups') 'active'}}"
              {{on "click" (fn this.setMode "groups")}}
            >Draw & teams</button>
          </div>
          ...
          {{#if (eq this.mode "wheel")}}
            ...
            <div class="settings-actions">
              <button
                type="button"
                class="btn active"
                disabled={{if this.entries.length this.spinning true}}
                {{on "click" this.spin}}
              >{{if this.spinning "Spinning…" "Spin"}}</button>
              <label class="math-check"><input
                  type="checkbox"
                  checked={{this.removeWinner}}
                  {{on "change" this.toggleRemove}}
                />
                Remove the winner after each spin</label>
            </div>
            {{#if this.winner}}
          ...
          {{else}}
            <div class="math-tabs" role="group" aria-label="Draw type">
              <button
                type="button"
                class="qr-tab {{if (eq this.groupMode 'teams') 'active'}}"
                {{on "click" (fn this.setGroupMode "teams")}}
              >Split into teams</button>
              <button
                type="button"
                class="qr-tab {{if (eq this.groupMode 'draw') 'active'}}"
                {{on "click" (fn this.setGroupMode "draw")}}
              >Draw names</button>
            </div>
            <div class="math-row is-aligned">
              <label class="math-field"><span class="qr-label is-muted">{{if
                    (eq this.groupMode "teams")
                    "Number of teams"
                    "How many to draw"
                  }}</span><input
                  type="number"
              ...
              <button
                ...
                disabled={{if this.entries.length false true}}
                {{on "click" this.makeGroups}}
              >{{if (eq this.groupMode "teams") "Make teams" "Draw"}}</button>
            </div>
            ...
            {{/if}}
          {{/if}}
          <p class="tool-hint">Picks use your browser's cryptographic random
            number generator, so every entry has an equal chance.</p>
        </section>
      </div>
    </ToolPage>
  </template>
