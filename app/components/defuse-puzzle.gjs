...
const PAD_DIRS = { DPadUp: 'up', DPadDown: 'down', DPadLeft: 'left', DPadRight: 'right' };
const ARROW_ICON = { up: 'arrow-up', down: 'arrow-down', left: 'arrow-left', right: 'arrow-right' };
...
export default class DefusePuzzle extends Component {
  ...
  puzzle = makePuzzle(this.args.defusing.seed, this.args.defusing.level, this.args.defusing.type);
  ...
  get wires() {
    return this.puzzle.wires.map((w, i) => ({ ...w, index: i, number: i + 1, style: htmlSafe(`--wire:${w.hex}`), selected: i === this.selected, cut: this.cut === i }));
  }

  get codeSlots() {
    return this.puzzle.digits.map((d, i) => ({ shown: i < this.progress ? d : this.codeHidden ? '•' : d, done: i < this.progress }));
  }
  ...
  get arrows() {
    ...
    return arrows.map((dir, i) => ({ icon: ARROW_ICON[dir], done: backwards ? i >= n - this.progress : i < this.progress }));
  }

  get zones() {
    return this.puzzle.zones.map(([a, b], i) => ({ style: htmlSafe(`left:${a * 100}%;width:${(b - a) * 100}%`), active: i === this.progress, done: i < this.progress }));
  }
  ...
  move(dir) {
    ...
    if (this.isWires) {
      ...
      if (dir === 'left' || dir === 'up') this.selected = (this.selected + n - 1) % n;
  ...
  setup = modifier((root) => {
    ...
    const onKey = (event) => {
      ...
      let used = true;
      if (digit && this.isWires) Number(digit) <= this.puzzle.wires.length && this.cutWire(Number(digit) - 1);
      else if (digit && this.isCode) this.pressDigit(Number(digit));
      else if (KEY_DIRS[event.code]) !event.repeat && this.move(KEY_DIRS[event.code]);
      else if (event.code === 'Space' || event.code === 'Enter') !event.repeat && this.confirm();
      else used = false;
      if (used) {
      ...
      {{#if this.isWires}}
        <div class="defuse-wires">
          {{#each this.wires as |w|}}
            <button type="button" class="defuse-wire {{if w.selected 'is-selected'}} {{if w.cut 'is-cut'}}" style={{w.style}} data-sound="off" aria-label="Wire {{w.number}}, {{w.id}}" {{on "click" (fn this.cutWire w.index)}}>
      ...
      {{else if this.isCode}}
        <div class="defuse-code">
          {{#each this.codeSlots as |slot|}}
            <span class="defuse-slot {{if slot.done 'is-done'}}">{{slot.shown}}</span>
          {{/each}}
        </div>
        <div class="defuse-keypad">
          {{#each this.keypad as |d i|}}
            <button type="button" class="btn defuse-key {{if (eq i this.selected) 'is-selected'}}" data-sound="off" {{on "click" (fn this.pressDigit d)}}>{{d}}</button>
          {{/each}}
        </div>
      {{else if this.isSequence}}
        <div class="defuse-sequence">
          {{#each this.arrows as |a|}}
            <span class="defuse-arrow {{if a.done 'is-done'}}"><Icon @name={{a.icon}} @size={{20}} /></span>
          {{/each}}
        </div>
        <div class="defuse-dpad">
          <button type="button" class="btn defuse-dir is-up" data-sound="off" aria-label="Up" {{on "click" (fn this.pressArrow "up")}}><Icon @name="arrow-up" @size={{18}} /></button>
          <button type="button" class="btn defuse-dir is-left" data-sound="off" aria-label="Left" {{on "click" (fn this.pressArrow "left")}}><Icon @name="arrow-left" @size={{18}} /></button>
          <button type="button" class="btn defuse-dir is-right" data-sound="off" aria-label="Right" {{on "click" (fn this.pressArrow "right")}}><Icon @name="arrow-right" @size={{18}} /></button>
          <button type="button" class="btn defuse-dir is-down" data-sound="off" aria-label="Down" {{on "click" (fn this.pressArrow "down")}}><Icon @name="arrow-down" @size={{18}} /></button>
        </div>
      {{else if this.isTiming}}
        <div class="defuse-meter">
          {{#each this.zones as |z|}}
            {{#if z.active}}<span class="defuse-zone" style={{z.style}}></span>{{/if}}
          {{/each}}
          <span class="defuse-needle"></span>
        </div>
        <button type="button" class="btn active defuse-stop" data-sound="off" {{on "click" this.stopNeedle}}>Stop{{#if (lt 1 this.hitsLeft)}} ({{this.hitsLeft}} left){{/if}}</button>
      {{/if}}
    </div>
  </template>
}
