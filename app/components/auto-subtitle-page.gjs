...
import { toCues, FORMATS, clock } from '../utils/subtitles';
...
const SIZES = [
  { id: 'whisper-tiny', label: 'Quick · English only (~40 MB)' },
  { id: 'whisper-base', label: 'Better · any language (~80 MB)' },
];
const eq = (a, b) => a === b;
...
export default class AutoSubtitlePage extends Component {
  ...
  transcribe = async () => {
    ...
    try {
      ...
      const wav = await runFFmpeg(this.file, {
        ...
        build: (input) => ['-i', input, '-vn', '-ac', '1', '-ar', '16000', '-c:a', 'pcm_s16le'],
        onProgress: (p) => (this.progress = p * 0.3),
      });
      this.status = isLoaded(this.size) ? 'Starting the speech model…' : 'Downloading the speech model (once, then it’s cached)…';
      ...
      this.progress = 1;
      const chunks = output?.chunks?.length ? output.chunks : [{ timestamp: [0, this.duration], text: output?.text ?? '' }];
      this.cues = toCues(chunks);
      if (!this.cues.length) this.error = 'Nothing was said, or the speech was too quiet to make out.';
    } catch (error) {
      this.error = error?.message ?? 'Couldn’t write the subtitles for this one';
    } finally {
      this.busy = false;
      this.status = '';
    }
  ...
  <template>
    <ToolPage @route="auto-subtitle" @subtitle="Writes the subtitles for a video or a recording, with timings — Whisper running on your device, so the file never leaves it.">
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          <label class="qr-drop fs-drop {{if this.dragging 'is-dragging'}}" {{on "dragover" this.dragOver}} {{on "dragleave" this.dragOver}} {{on "drop" this.drop}}>
            ...
            <span>{{if this.dragging "Drop it here" "Drop a video or a recording, paste it, or click to browse"}}</span>
          ...
          <div class="math-tabs" role="group" aria-label="Model size">
            {{#each this.sizes as |s|}}
              <button type="button" class="qr-tab {{if (eq this.size s.id) 'active'}}" {{on "click" (fn this.pick "size" s.id)}}>{{s.label}}</button>
            {{/each}}
          </div>
          <p class="tool-hint">The model downloads once from Hugging Face and is cached by your browser after that; the audio itself is never sent anywhere. Expect roughly real time on a laptop — a ten-minute video takes a few minutes.</p>

          {{#if this.file}}
            <div class="fc-toolbar">
              <span class="tool-hint">{{this.file.name}} · {{formatBytes this.file.size}}{{#if this.duration}} · {{formatTime this.duration}}{{/if}}</span>
              <div class="settings-actions">
                <button type="button" class="btn active" disabled={{this.busy}} {{on "click" this.transcribe}}>{{if this.busy "Working…" "Write the subtitles"}}</button>
        ...
        {{#if this.cues.length}}
          <div class="fs-frame fc-panel pop-in">
            <div class="fc-toolbar">
              ...
              <div class="settings-actions">
                <select class="select" aria-label="Subtitle format" {{on "change" this.setFormat}}>
                  {{#each this.formats as |f|}}
                    <option value={{f.id}} selected={{eq this.format f.id}}>{{f.label}}</option>
                  {{/each}}
                </select>
                <button type="button" class="btn active" {{on "click" this.save}}><Icon @name="download" @size={{13}} /> Save</button>
            ...
            {{#if this.mediaUrl}}
              ...
              {{#if this.isAudio}}
                <audio class="trim-player" controls src={{this.mediaUrl}}></audio>
              {{else}}
                <video class="trim-player" controls src={{this.mediaUrl}}></video>
              {{/if}}
            {{/if}}
