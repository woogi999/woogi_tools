...
const STOPWORDS = new Set(
  'a an and are as at be but by for from has have he her his i if in into is it its of on or our she so that the their them then there these they this to was we were what when which who will with you your not no do does did can could would should than too very just about also'.split(
    ' ',
  ),
);
...
export default class WordCounterPage extends Component {
  ...
  get stats() {
    ...
    return [
      ...
      { label: 'Characters', value: [...text].length.toLocaleString() },
      {
        ...
        value: [...text.replace(/\s/g, '')].length.toLocaleString(),
      },
      {
        label: 'Sentences',
        value: (text.match(/[^.!?。！？]+[.!?。！？]+|[^.!?。！？]+$/g) ?? [])
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'word-counter', ['text']);
  }

  <template>
    <ToolPage
      @route="word-counter"
      @subtitle="Type or paste your text for words, characters, sentences and reading time, live as you go."
    >
        ...
        <section class="math-card">
          ...
          <div class="math-stats">
            {{#each this.stats as |s|}}
              <div class="math-stat"><span>{{s.label}}</span><strong
                >{{s.value}}</strong></div>
            {{/each}}
          </div>
          ...
          {{#if this.keywords.length}}
            <ol class="wc-keywords">
              {{#each this.keywords as |k|}}
                <li><span>{{k.word}}</span><span class="tool-hint">{{k.count}}
                    ·
                    {{k.share}}</span></li>
              {{/each}}
            </ol>
