...
export default class VideoMuterPage extends Component {
  ...
  async addFiles(list) {
    const files = [...list].filter((f) => f.type.startsWith('video/') || /\.(mp4|mkv|mov|webm|avi|m4v|flv|wmv|ts|mpg|mpeg|gif)$/i.test(f.name));
    if (!files.length) return;
    const added = files.map((file) => ({ id: nextId++, file, url: null, name: null, size: null, status: '', progress: 0, error: null, duration: 0, hasAudio: undefined }));
    this.items = [...this.items, ...added];
    for (const item of added) {
      const info = await mediaInfo(item.file);
      this.update(item.id, { duration: info.duration, hasAudio: info.hasAudio });
    }
  }
  ...
  mute = async (id) => {
    ...
    try {
      ...
      this.update(id, { url: URL.createObjectURL(blob), name: `${baseName(item.file.name)}-muted.${ext}`, size: blob.size, status: '', progress: 1 });
    } catch (error) {
      this.update(id, { error: error?.message || 'Couldn’t mute this one', status: '' });
    }
  };

  <template>
    <ToolPage @route="video-muter" @subtitle="Strip the sound off a video and keep the picture exactly as it was. Quick, lossless and all on your device.">
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          <label class="qr-drop fs-drop {{if this.dragging 'is-dragging'}}" {{on "dragover" this.dragOver}} {{on "dragleave" this.dragOver}} {{on "drop" this.drop}}>
            ...
            <input type="file" accept="video/*" multiple class="sr-only" {{on "change" this.selectFiles}} />
          </label>
          <p class="tool-hint">The picture is copied over frame for frame, so nothing is re-encoded and nothing is lost: only the sound goes. The engine downloads the first time you use it (about 30 MB), then stays cached for the visit.</p>
        </div>
          ...
          <div class="fs-frame fc-panel pop-in">
            <div class="fc-toolbar">
              ...
              <div class="settings-actions">
                <button type="button" class="btn active" disabled={{this.busy}} {{on "click" this.muteAll}}>{{if this.busy "Working…" "Mute them"}}</button>
                ...
                <li class="fs-row fc-row">
                  ...
                  <div class="fs-row-status">
                    {{#if item.url}}
                      ...
                      <video class="media-preview" controls src={{item.url}}></video>
                    ...
                    {{else}}
                      <button type="button" class="btn" {{on "click" (fn this.mute item.id)}}>{{if item.error "Retry" "Mute"}}</button>
                    {{/if}}
                  </div>
