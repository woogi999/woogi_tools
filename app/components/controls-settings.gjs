...
export default class ControlsSettings extends Component {
  ...
  listen = (action, kind) => {
    this.listening = this.isListening(action, kind)
      ? null
      : { game: this.game, action, kind };
  };
  ...
  capture = modifier(() => {
    ...
    const release = pushPadHandler((button, { down }) => {
      ...
      if (PAD_BUTTONS.includes(button))
        keybinds.assign(l.game, l.action, 'pad', button);
      this.listening = null;
      return true;
    });
    return () => {
    ...
    <div class="controls-settings" {{this.capture}}>
      <p class="tool-hint">
        {{#if this.pad.connected}}
          ...
          {{this.pad.connected}}
          connected. Use the D-pad to move round the site, A to press, B to go
          back, Start for search.
        {{else}}
          <Icon @name="gamepad-2" @size={{13}} />
          Plug in or pair a controller and press any button: it works across the
          whole site, not just the games.
        {{/if}}
      </p>
      ...
      <div class="math-tabs" role="tablist" aria-label="Game">
        {{#each this.games key="game" as |g|}}
          <button
            type="button"
            role="tab"
            class="qr-tab {{if (eq g.game this.game) 'active'}}"
            aria-selected={{if (eq g.game this.game) "true" "false"}}
            {{on "click" (fn this.pickGame g.game)}}
          >{{g.label}}</button>
        {{/each}}
      </div>
      ...
      <div
        ...
        role="table"
        aria-label="{{this.current.label}} controls"
      >
        <div class="controls-row is-head" role="row">
          <span role="columnheader">Action</span>
          <span role="columnheader"><Icon @name="keyboard" @size={{13}} />
            Keys</span>
          <span role="columnheader"><Icon @name="gamepad-2" @size={{13}} />
            Controller</span>
        </div>
          ...
          <div class="controls-row" role="row">
            ...
            <span class="controls-binds" role="cell">
              {{#each a.keyList key="code" as |k|}}
                <span class="controls-chip"><kbd>{{k.label}}</kbd><button
                    type="button"
            ...
            <span class="controls-binds" role="cell">
              {{#each a.padList key="button" as |b|}}
                <span class="controls-chip"><kbd>{{b.label}}</kbd><button
                    type="button"
                    class="controls-chip-remove"
              ...
              {{#if this.pad.connected}}
                <button
                  type="button"
                  class="controls-add {{if a.waitingPad 'is-listening'}}"
                  {{on "click" (fn this.listen a.id "pad")}}
                >{{if a.waitingPad "Press a button…" "+ Button"}}</button>
              {{else}}
                <select
      ...
      <p class="tool-hint">A key or button does one thing per game: giving it to
        one action takes it off any other. Esc cancels while choosing.</p>
      <button
        type="button"
        class="btn math-use"
        disabled={{if this.current.customised false true}}
        {{on "click" this.reset}}
