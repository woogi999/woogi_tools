...
const ALL_ZONES = typeof Intl.supportedValuesOf === 'function' ? Intl.supportedValuesOf('timeZone') : ['UTC', 'America/New_York', 'Europe/London', 'Asia/Tokyo'];
const DEFAULT_ZONES = ['UTC', 'America/Los_Angeles', 'America/New_York', 'Europe/London', 'Asia/Manila', 'Asia/Tokyo'];
...
function parseInput(raw) {
  ...
  return Number.isNaN(date.getTime()) ? { error: "Couldn't read that as a timestamp or date" } : { date, unit: 'date text' };
}
...
export default class TimestampConverterPage extends Component {
  ...
  @tracked shownZones = DEFAULT_ZONES.includes(LOCAL_ZONE) ? DEFAULT_ZONES : [LOCAL_ZONE, ...DEFAULT_ZONES];
  ...
  get rows() {
    ...
    return [
      ...
      { label: `Local (${LOCAL_ZONE})`, value: date.toLocaleString(undefined, { dateStyle: 'full', timeStyle: 'long' }) },
      { label: 'Relative', value: relative(ms, this.now) },
      { label: 'Day of year', value: String(Math.floor((date - start) / 86400000)) },
  ...
  get zoneRows() {
    const date = this.parsed?.date;
    if (!date || Number.isNaN(date.getTime())) return [];
    return this.shownZones.map((zone) => ({ zone, value: zoneFormatter(zone).format(date) }));
  }
        ...
        <section class="math-card">
          ...
          <div class="math-row is-aligned">
            <label class="math-field">
              ...
              <input type="text" class="math-input" spellcheck="false" placeholder="1700000000, 2025-01-31T12:00Z, March 5 2026…" value={{this.input}} {{on "input" this.setInput}} />
            </label>
          ...
          <ul class="case-list">
            {{#each this.rows key="label" as |r|}}
              <li class="case-item">
                <div class="case-text"><span class="qr-label is-muted">{{r.label}}</span><span class="case-value is-mono">{{r.value}}</span></div>
        ...
        <section class="math-card">
          ...
          <ul class="case-list">
            {{#each this.zoneRows key="zone" as |z|}}
              <li class="case-item">
                <div class="case-text"><span class="qr-label is-muted">{{z.zone}}</span><span class="case-value">{{z.value}}</span></div>
                <button type="button" class="fs-remove" aria-label="Remove {{z.zone}}" {{on "click" (fn this.removeZone z.zone)}}><Icon @name="x" @size={{13}} /></button>
              </li>
            {{/each}}
          </ul>
          <select class="select" aria-label="Add a time zone" {{on "change" this.addZone}}>
