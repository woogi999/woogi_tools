...
function birthdayIn(year, birth) {
  ...
  return new Date(
    Date.UTC(
      year,
      month,
      Math.min(birth.getUTCDate(), daysInMonth(year, month)),
    ),
  );
}
...
export default class AgeCalculatorPage extends Component {
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'age-calculator', ['birth']);
  }
  ...
  get result() {
    ...
    if (birth > asOf)
      return { error: 'The birth date is after the "age on" date.' };
  ...
  <template>
    <ToolPage
      ...
      @subtitle="Pick a birthday for the exact age, some fun totals, and a countdown to the next birthday."
    >
      <div class="math-grid pop-in">
        <section class="math-card">
          <div class="math-row">
            <label class="math-field"><span class="qr-label is-muted">Date of
                birth</span><input
                type="date"
                class="math-input"
                value={{this.birth}}
                {{on "input" this.setBirth}}
              /></label>
        ...
        {{#unless this.result.error}}
          <section class="math-card">
            <h3 class="qr-heading">{{if
                this.result.isBirthday
                "Today's the day"
                "Next birthday"
              }}</h3>
            {{#if this.result.isBirthday}}
              <p class="math-callout">Happy birthday! You turn
                <strong>{{this.result.nextAge}}</strong>
                today 🎂</p>
            {{else}}
              <p class="math-callout">Turning
                <strong>{{this.result.nextAge}}</strong>
                in
                {{this.result.nextText}}</p>
            {{/if}}
            <p class="tool-hint">{{this.result.nextDate}}</p>
            <div class="math-stats">
              <div class="math-stat"><span>Born on a</span><strong
                >{{this.result.bornOn}}</strong></div>
              <div class="math-stat"><span>Star sign</span><strong
                >{{this.result.zodiac}}</strong></div>
              <div class="math-stat"><span>Chinese zodiac (by year)</span><strong
                >{{this.result.chinese}}</strong></div>
            </div>
          </section>
        {{/unless}}
      </div>
