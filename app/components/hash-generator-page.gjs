...
export default class HashGeneratorPage extends Component {
  ...
  get display() {
    return this.hashes.map((h) => ({
      ...h,
      value: this.uppercase ? h.value.toUpperCase() : h.value,
    }));
  }
  ...
  <template>
    ...
    >
      <div
        class="math-grid text-tool pop-in"
      ...
      >
        <section class="math-card">
          <div class="math-tabs" role="group" aria-label="Input">
            <button
              type="button"
              class="qr-tab {{if (eq this.source 'text') 'active'}}"
              {{on "click" (fn this.setSource "text")}}
            >Text</button>
            <button
              type="button"
              class="qr-tab {{if (eq this.source 'file') 'active'}}"
              {{on "click" (fn this.setSource "file")}}
            >File</button>
          </div>
          ...
          {{else}}
            <label
              class="qr-drop {{if this.dragging 'is-dragging'}}"
            ...
            >
              ...
              {{#if this.file}}<span class="tool-hint">{{formatBytes
                    this.file.size
                  }}</span>{{/if}}
              <input
                type="file"
            ...
            <p class="tool-hint">The file is read on your device and never
              uploaded.</p>
          {{/if}}
