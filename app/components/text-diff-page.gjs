...
export default class TextDiffPage extends Component {
        ...
        runs
          ...
          .reduce(
            (n, r) =>
              n +
              (this.mode === 'words'
                ? r.value.split(/\s+/).filter(Boolean).length
                : r.count),
            0,
          );
      return {
        ...
        unit: this.mode === 'words' ? 'word' : 'character',
      };
    } catch (error) {
  ...
  get identical() {
    return (
      !this.result.error && this.result.added === 0 && this.result.removed === 0
    );
  }
  ...
  <template>
    <ToolPage
      @route="text-diff"
      @subtitle="Paste two versions of something and see exactly what changed, word by word or line by line."
    >
        ...
        <section class="math-card">
          <div class="fc-toolbar">
            ...
            <div class="settings-actions">
              ...
              <label class="math-check"><input
                  type="checkbox"
                  checked={{this.ignoreWhitespace}}
                  {{on "change" this.toggleWhitespace}}
                />
          ...
          {{else}}
            <p class="tool-hint">
              {{#if this.identical}}No differences.{{else}}<span
                  class="diff-count is-ins"
                >+{{this.result.added}}</span>
                <span class="diff-count is-del">−{{this.result.removed}}</span>
                {{this.result.unit}}s{{/if}}
            </p>
            {{#if this.result.lines}}
              <ol class="diff-lines">
                {{#each this.result.lines as |line|}}
                  <li class="diff-line is-{{line.type}}"><span
                      class="diff-gutter"
                      aria-hidden="true"
                    >{{sign line.type}}</span><span
                      class="diff-text"
                    >{{line.value}}</span></li>
                {{/each}}
              </ol>
            {{else}}
              <div class="diff-inline">{{#each this.result.runs as |run|}}{{#if
                    (eq run.type "insert")
                  }}<ins>{{run.value}}</ins>{{else if
                    (eq run.type "delete")
                  }}<del>{{run.value}}</del>{{else}}<span
                    >{{run.value}}</span>{{/if}}{{/each}}</div>
            {{/if}}
          {{/if}}
        </section>
      </div>
