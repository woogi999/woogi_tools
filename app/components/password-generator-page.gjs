...
function generatePassword(length, sets, excludeAmbiguous) {
  ...
  cleaned.forEach((set, i) => {
    if (i < length && !chars.some((c) => set.includes(c)))
      chars[randomIndex(length)] = randomChar(set);
  });
  return chars.join('');
}
...
function assessPassword(password) {
  ...
  if (COMMON_PASSWORDS.has(password.toLowerCase())) {
    return {
      ...
      crackTime: 'instantly',
      rating: 'weak',
      ratingLabel: 'Very weak: one of the most common passwords',
      percent: 5,
    };
  }
  ...
  if (entropy < 28) [rating, ratingLabel, percent] = ['weak', 'Very weak', 15];
    ...
    [rating, ratingLabel, percent] = ['fair', 'Reasonable', 55];
  else if (entropy < 100)
    [rating, ratingLabel, percent] = ['strong', 'Strong', 80];
...
export default class PasswordGeneratorPage extends Component {
  ...
  constructor(owner, args) {
    super(owner, args);
    keepState(this, 'password-generator', [
      'length',
      'useLower',
      'useUpper',
      'useNumbers',
  ...
  <template>
    <ToolPage
      ...
      @subtitle="Generate a strong random password, or test the one you already use (it never leaves your device)."
    >
      <div class="math-grid pop-in">
        <section class="math-card">
          ...
          <div class="math-row is-aligned">
            ...
            ><Icon @name="refresh-cw" @size={{13}} /> New</button>
          </div>

          <div class="pw-meter" data-rating={{this.assessment.rating}}>
            <div class="pw-meter-bar"><div
                class="pw-meter-fill"
                style={{meterWidth this.assessment.percent}}
              ></div></div>
            <span class="tool-hint">{{this.assessment.ratingLabel}}
              · would take about
              {{this.assessment.crackTime}}
              to crack</span>
          </div>
          ...
          <div class="math-row">
            <label class="math-check"><input
                type="checkbox"
                checked={{this.useLower}}
                {{on "change" this.toggleLower}}
              />
              Lowercase (a-z)</label>
            <label class="math-check"><input
                type="checkbox"
                checked={{this.useUpper}}
                {{on "change" this.toggleUpper}}
              />
              Uppercase (A-Z)</label>
            <label class="math-check"><input
                type="checkbox"
                checked={{this.useNumbers}}
                {{on "change" this.toggleNumbers}}
              />
              Numbers (0-9)</label>
            <label class="math-check"><input
                type="checkbox"
                checked={{this.useSymbols}}
                {{on "change" this.toggleSymbols}}
              />
              Symbols (!@#…)</label>
            <label class="math-check"><input
                type="checkbox"
                checked={{this.excludeAmbiguous}}
                {{on "change" this.toggleAmbiguous}}
              />
              Exclude ambiguous (Il1O0o)</label>
          </div>
        </section>
        ...
        <section class="math-card">
          ...
          <div class="math-row is-aligned">
            <input
              type={{if this.showCheckValue "text" "password"}}
              class="math-input"
              placeholder="Paste or type a password"
              aria-label="Password to check"
              value={{this.checkValue}}
              {{on "input" this.setCheckValue}}
              autocomplete="off"
            />
            <button
              type="button"
              class="btn math-use"
              {{on "click" this.toggleShowCheck}}
            ><Icon
                @name={{if this.showCheckValue "eye-off" "eye"}}
                @size={{13}}
              /></button>
          </div>
          <div class="pw-meter" data-rating={{this.checkAssessment.rating}}>
            <div class="pw-meter-bar"><div
                class="pw-meter-fill"
                style={{meterWidth this.checkAssessment.percent}}
              ></div></div>
            <span class="tool-hint">{{this.checkAssessment.ratingLabel}}{{#if
                this.checkValue
              }}
                · about
                {{this.checkAssessment.entropy}}
                bits of entropy · would take about
                {{this.checkAssessment.crackTime}}
                to crack{{/if}}</span>
          </div>
          <p class="tool-hint">Nothing you type here ever leaves your browser;
            the check runs entirely on your device.</p>
        </section>
      </div>
    </ToolPage>
  </template>
