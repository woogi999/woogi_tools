...
const SAMPLE =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6Ildvb2dpIiwiaWF0IjoxNzAwMDAwMDAwLCJleHAiOjQxMDI0NDQ4MDB9.XpVA21w55jbzB8c9Kp6nNDGsHYug3JZVZJRxN6uYEfE';
...
const decodePart = (part) =>
  JSON.parse(new TextDecoder().decode(base64UrlBytes(part)));
...
export default class JwtDecoderPage extends Component {
  ...
  get headerJson() {
    return this.parsed?.header
      ? JSON.stringify(this.parsed.header, null, 2)
      : '';
  }

  get payloadJson() {
    return this.parsed?.payload
      ? JSON.stringify(this.parsed.payload, null, 2)
      : '';
  }
  ...
  get claims() {
    ...
    return Object.entries(payload)
      ...
      .map(([key, value]) => ({
        ...
        value:
          TIME_CLAIMS[key] && typeof value === 'number'
            ? `${new Date(value * 1000).toLocaleString()} · ${relative(value * 1000)}`
            : Array.isArray(value)
              ? value.join(', ')
              : String(value),
      }));
  ...
  get status() {
    ...
    if (typeof payload.exp === 'number' && payload.exp < now)
      return { ok: false, text: `Expired ${relative(payload.exp * 1000)}.` };
    if (typeof payload.nbf === 'number' && payload.nbf > now)
      return {
        ok: false,
        text: `Not valid until ${relative(payload.nbf * 1000)}.`,
      };
    return {
      ok: true,
      text:
        typeof payload.exp === 'number'
          ? `Not expired; expires ${relative(payload.exp * 1000)}.`
          : 'No expiry set.',
    };
  }
  ...
  async verify() {
    ...
    const key = await crypto.subtle.importKey(
      ...
      { name: 'HMAC', hash: HMAC[this.algorithm] },
      false,
      ['verify'],
    );
    let valid = false;
  ...
  <template>
    <ToolPage
      @route="jwt-decoder"
      @subtitle="Paste a JWT to read its header and claims, see when it expires, and verify HMAC signatures. All in your browser."
    >
      <div class="math-grid text-tool pop-in">
        <section class="math-card">
          ...
          {{else if this.status}}
            <p
              class="math-callout {{unless this.status.ok 'is-soft'}}"
            >{{this.status.text}}</p>
          {{/if}}
          <p class="tool-hint">Decoding happens in your browser. Even so, avoid
            pasting live production tokens into any website.</p>
            ...
            {{#if this.canVerify}}
              <label class="math-field">
                ...
                <input
                  ...
                  value={{this.secret}}
                  {{on "input" this.setSecret}}
                />
              </label>
              {{#if this.secret}}
                <p
                  class="math-callout {{unless this.verification 'is-soft'}}"
                >{{if
                    this.verification
                    "Signature verified."
              ...
              {{/if}}
            {{else}}
              <p class="tool-hint">Only HMAC tokens (HS256/384/512) can be
                verified here;
                {{this.algorithm}}
                needs the issuer's public key.</p>
            {{/if}}
          {{/if}}
        </section>
        ...
        {{#if this.parsed.payload}}
          <section class="math-card">
            {{#if this.claims.length}}
              <div class="math-stats">
                {{#each this.claims as |c|}}
                  <div class="math-stat"><span>{{c.label}}</span><strong
                    >{{c.value}}</strong></div>
                {{/each}}
              </div>
            {{/if}}
            <div class="field-head"><span
              ...
              >Header</span><CopyButton @value={{this.headerJson}} /></div>
            <pre class="code-block">{{this.headerJson}}</pre>
            <div class="field-head"><span
                class="qr-label is-muted"
              >Payload</span><CopyButton @value={{this.payloadJson}} /></div>
