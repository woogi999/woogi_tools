...
export default class BackgroundRemoverPage extends Component {
  ...
  addFiles(list) {
    ...
    this.items = [...this.items, ...images.map((file) => ({ id: nextId++, file, sourceUrl: URL.createObjectURL(file), url: null, size: null, error: null }))];
  }
        ...
        try {
          ...
          const blob = await removeBackground(item.file, {
            ...
            progress: (key, current, total) => {
              if (total) this.status = `${key}: ${Math.round((current / total) * 100)}%`;
            },
          });
          const url = URL.createObjectURL(blob);
          const name = `${item.file.name.replace(/\.[^.]+$/, '')}-no-bg.png`;
          this.items = this.items.map((i) => (i.id === item.id ? { ...i, url, size: blob.size, name, error: null } : i));
        } catch (error) {
          this.items = this.items.map((i) => (i.id === item.id ? { ...i, error: error.message || 'Failed to process this image.' } : i));
        }
      }
      this.status = null;
    } finally {
          ...
          <label class="math-field">
            ...
            <div class="math-tabs" role="group" aria-label="Model">
              {{#each this.models as |m|}}
                <button type="button" class="qr-tab {{if (eq this.model m.id) 'active'}}" {{on "click" (fn this.setModel m.id)}}>{{m.label}}</button>
              {{/each}}
            </div>
          </label>
          <p class="tool-hint">The first run downloads a segmentation model (a few MB to tens of MB depending on quality) that your browser caches for next time. Nothing is uploaded — the model runs entirely on your device.</p>
        </div>
          ...
          <div class="fs-frame fc-panel pop-in">
            <div class="fc-toolbar">
              <div class="settings-actions">
                <button type="button" class="btn active" disabled={{this.busy}} {{on "click" this.run}}>{{if this.busy "Working…" "Remove backgrounds"}}</button>
                <button type="button" class="btn" disabled={{this.busy}} {{on "click" this.clear}}>Clear</button>
              </div>
              {{#if this.status}}<span class="tool-hint">{{this.status}}</span>{{/if}}
            </div>
            <ul class="fs-list">
              {{#each this.items key="id" as |item|}}
                <li class="fs-row fc-row bgr-row">
                  <img src={{item.sourceUrl}} alt="" class="bgr-thumb" />
                  {{#if item.url}}<img src={{item.url}} alt="" class="bgr-thumb checkerboard" />{{/if}}
