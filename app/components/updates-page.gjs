...
const TYPE_LABELS = { new: 'New', improved: 'Improved', fixed: 'Fixed', removed: 'Removed' };

const formatDate = (iso) => new Date(`${iso}T00:00:00`).toLocaleDateString(undefined, { dateStyle: 'long' });
...
export default class UpdatesPage extends Component {
  ...
  releases = CHANGELOG.map((release) => ({
    ...
    changes: release.changes.map((c) => ({ ...c, label: TYPE_LABELS[c.type] ?? c.type })),
  }));
    ...
    <div class="container">
      <section class="hero pop-in">
        ...
        <div class="hero-text">
          ...
          <p>What's changed on Woogi Tools. You're on version {{APP_VERSION}}{{#if this.offline.buildId}} (build <code>{{this.offline.buildId}}</code>){{/if}}.</p>
        </div>
      </section>
      ...
      {{#if this.offline.updateReady}}
        <div class="updates-ready pop-in">
          ...
          <button type="button" class="btn math-use" {{on "click" this.offline.reload}}><Icon @name="refresh-cw" @size={{13}} /> Reload</button>
        </div>
      {{/if}}
        ...
        {{#each this.releases key="version" as |release|}}
          <li class="updates-release">
            <header class="updates-header">
              ...
              {{#if release.isCurrent}}<span class="updates-badge">Current</span>{{/if}}
              <time class="tool-hint" datetime={{release.date}}>{{release.dateLabel}}</time>
            </header>
            <h3 class="qr-heading">{{release.title}}</h3>
            <ul class="updates-changes">
              {{#each release.changes as |change|}}
                <li class="updates-change">
                  <span class="updates-type is-{{change.type}}">{{change.label}}</span>
