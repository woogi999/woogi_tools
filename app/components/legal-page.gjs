...
function renderMarkdown(source) {
  const inline = (text) =>
    escape(text).replace(/\[([^\]]+)\]\(((?:https?:|mailto:)[^)\s]+)\)/g, '<a href="$2">$1</a>');
  return source
    .split(/\r?\n\s*\r?\n/)
    ...
    .map((block) => {
      ...
      if (/^Last updated/i.test(block)) return `<p class="legal-updated">${inline(block)}</p>`;
  ...
  <template>
    <div class="container legal-page">
      ...
      <article class="legal-document">
        ...
        {{else if this.failed}}
          <h1>{{@title}}</h1>
          <p>Couldn't load this page. <a href="/legal/{{@route}}.md">Open the document directly</a>.</p>
        {{else}}
          <h1>{{@title}}</h1>
