...
export default class FileConverterPage extends Component {
  ...
  constructor(owner, args) {
    ...
    registerDestructor(this, () => {
      for (const item of this.items) if (item.resultUrl) URL.revokeObjectURL(item.resultUrl);
    });
  }
  ...
  reset = (id) => {
    const item = this.items.find((it) => it.id === id);
    if (item?.resultUrl) URL.revokeObjectURL(item.resultUrl);
    this.update(id, { status: 'ready', resultUrl: null, resultName: null, resultSize: null, error: null, progress: 0 });
  };

  convert = async (id) => {
    const item = this.items.find((it) => it.id === id);
    if (!item?.target || item.status === 'working') return;
    this.update(id, { status: 'working', statusText: 'Starting…', progress: 0, error: null });
    try {
      ...
      this.update(id, { status: 'done', progress: 1, resultUrl: URL.createObjectURL(result.blob), resultName: result.name, resultSize: result.blob.size });
    } catch (error) {
      this.update(id, { status: 'error', error: error?.message || 'Conversion failed' });
    }
  };
    ...
    <ToolPage @route="file-converter" @subtitle="Images, RAW photos, audio, video, documents, data, archives, fonts… pick a format and convert. It all runs on your device.">
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
            ...
            <div class="fc-toolbar">
              ...
              <div class="settings-actions">
                {{#if this.canConvertAll}}
                  <button type="button" class="btn active" {{on "click" this.convertAll}}>Convert all</button>
                {{/if}}
                <button type="button" class="btn" {{on "click" this.clearAll}}>Clear</button>
              </div>
            </div>
            <ul class="fs-list">
              {{#each this.items key="id" as |item|}}
                <ConvertRow @item={{item}} @onSetTarget={{fn this.setTarget item.id}} @onConvert={{fn this.convert item.id}} @onReset={{fn this.reset item.id}} @onRemove={{fn this.remove item.id}} />
              {{/each}}
            </ul>
          </div>
        {{/if}}
        ...
        <div class="fc-panel pop-in">
          ...
          <p class="tool-hint">Anything below can be read. If there's no single tool for a pair, the converter chains a few together (DOCX → text → PDF, say) and shows the route.</p>
          <dl class="fc-formats">
  ...
  <li class="fs-row fc-row">
    ...
    <div class="fs-row-info">
      ...
      <span class="fs-row-size">
        {{if @item.source @item.source.label "Unknown type"}} · {{formatBytes @item.file.size}}{{#if @item.resultSize}} → {{formatBytes @item.resultSize}}{{/if}}
      </span>
    ...
    <div class="fs-row-status">
      {{#if (eq @item.status "unsupported")}}
        <span class="fs-tag is-error"><Icon @name="circle-alert" @size={{13}} /> Not supported yet</span>
      {{else if (eq @item.status "working")}}
          ...
          {{#each @item.groups as |group|}}
            <optgroup label={{group.label}}>
              {{#each group.formats as |f|}}
                <option value={{f.ext}} selected={{eq @item.target f.ext}}>{{f.label}}</option>
              {{/each}}
            </optgroup>
          {{/each}}
        </select>
        <button type="button" class="btn active" {{on "click" @onConvert}}>{{if (eq @item.status "error") "Retry" "Convert"}}</button>
      {{/if}}
    </div>
    <button type="button" class="fs-remove" aria-label="Remove {{@item.file.name}}" {{on "click" @onRemove}}><Icon @name="x" @size={{13}} /></button>
  </li>
</template>;
...
function via(item) {
  return item.source && item.target ? routeDescription(item.source.ext, item.target) : null;
}
