...
import { runFFmpeg, baseName, mediaInfo, formatTime, ffmpegTime } from '../utils/media-jobs';
...
export default class TrimmerPage extends Component {
  ...
  async open(file) {
    ...
    this.isAudio = file.type.startsWith('audio/') || /\.(mp3|wav|flac|ogg|m4a|opus|aac)$/i.test(file.name);
  ...
  setStart = (e) => {
    this.start = clamp(Number(e.target.value) || 0, 0, Math.max(0, this.end - 0.1));
    this.seek(this.start);
  };

  setEnd = (e) => {
    this.end = clamp(Number(e.target.value) || 0, this.start + 0.1, this.duration);
  };
  ...
  markStart = () => {
    this.start = clamp(this.player?.currentTime ?? 0, 0, Math.max(0, this.end - 0.1));
  };

  markEnd = () => {
    this.end = clamp(this.player?.currentTime ?? this.duration, this.start + 0.1, this.duration);
  };
  ...
  reset = () => {
    this.clearUrls();
    this.file = this.url = this.resultUrl = this.resultName = this.resultSize = null;
  ...
  cut = async () => {
    ...
    try {
      const blob = await runFFmpeg(this.file, {
        ...
        build: (input) => ['-ss', ffmpegTime(this.start), '-to', ffmpegTime(this.end), '-i', input, ...(exact ? ['-c:a', 'aac'] : ['-c', 'copy']), '-avoid_negative_ts', 'make_zero'],
  ...
  <template>
    <ToolPage @route="trimmer" @subtitle="Cut a piece out of a video or a song. Pick the start and end, hear or see it first, then save just that bit.">
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          <label class="qr-drop fs-drop {{if this.dragging 'is-dragging'}}" {{on "dragover" this.dragOver}} {{on "dragleave" this.dragOver}} {{on "drop" this.drop}}>
            ...
            <span>{{if this.dragging "Drop it here" "Drop a video or an audio file, paste it, or click to browse"}}</span>
            <input type="file" accept="video/*,audio/*" class="sr-only" {{on "change" this.selectFile}} />
          </label>
          {{#unless this.file}}
            <p class="tool-hint">Works with videos and music alike. The engine downloads the first time you use it (about 30 MB), then stays cached for the visit.</p>
          {{/unless}}
        </div>
        ...
        {{#if this.file}}
          <div class="fs-frame fc-panel pop-in">
            <div class="fc-toolbar">
              ...
              <span class="tool-hint">{{formatBytes this.file.size}} · {{formatTime this.duration}}</span>
            </div>

            {{#if this.isAudio}}
              {{! template-lint-disable require-media-caption }}
              <audio class="trim-player" controls src={{this.url}} {{this.keepPlayer}} {{on "timeupdate" this.watch}}></audio>
            {{else}}
              {{! template-lint-disable require-media-caption }}
              <video class="trim-player" controls src={{this.url}} {{this.keepPlayer}} {{on "timeupdate" this.watch}}></video>
            {{/if}}
            ...
            <label class="math-field">
              <span class="qr-label is-muted">Start: {{formatTime this.start}}</span>
              <input type="range" min="0" max={{this.duration}} step="0.05" value={{this.start}} {{on "input" this.setStart}} />
            </label>
            <label class="math-field">
              <span class="qr-label is-muted">End: {{formatTime this.end}}</span>
              <input type="range" min="0" max={{this.duration}} step="0.05" value={{this.end}} {{on "input" this.setEnd}} />
            </label>

            <div class="settings-actions">
              <button type="button" class="btn" {{on "click" this.markStart}}><Icon @name="circle-dot" @size={{13}} /> Start here</button>
              <button type="button" class="btn" {{on "click" this.markEnd}}><Icon @name="circle-dot" @size={{13}} /> End here</button>
              <button type="button" class="btn" {{on "click" this.playSelection}}><Icon @name="play" @size={{13}} /> Play the bit</button>
            </div>

            <div class="math-tabs" role="group" aria-label="How to cut">
              <button type="button" class="qr-tab {{if (eq this.mode 'copy') 'active'}}" {{on "click" (fn this.setMode "copy")}}>Quick (no re-encoding)</button>
              <button type="button" class="qr-tab {{if (eq this.mode 'exact') 'active'}}" {{on "click" (fn this.setMode "exact")}}>Exact cut</button>
            </div>
            <p class="tool-hint">
              {{if
                (eq this.mode "copy")
            ...
            <div class="fc-toolbar">
              <div class="settings-actions">
                <button type="button" class="btn active" disabled={{if this.canCut false true}} {{on "click" this.cut}}>{{if this.busy "Cutting…" "Cut it"}}</button>
                <button type="button" class="btn" {{on "click" this.reset}}>Start over</button>
              </div>
              <span class="tool-hint">Keeping {{formatTime this.span}} of {{formatTime this.duration}}</span>
            </div>

            {{#if this.busy}}
              <div class="fs-progress"><div class="fs-progress-bar" style={{progressWidth this.progress}}></div></div>
            ...
            {{#if this.resultUrl}}
              <div class="fs-row fc-row">
                ...
                <div class="fs-row-info">
                  ...
                  <span class="fs-row-size">{{formatBytes this.resultSize}} · {{formatTime this.span}}</span>
                </div>
