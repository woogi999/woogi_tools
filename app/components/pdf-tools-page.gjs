...
async function readPdf(file) {
  ...
  try {
    ...
    throw new Error(
      /encrypt/i.test(error.message)
        ? `${file.name} is password-protected`
        : `${file.name} isn't a readable PDF`,
    );
  }
}
...
export default class PdfToolsPage extends Component {
  ...
  get pipWarning() {
    return 'Close PDF Tools? The job in progress will be cancelled.';
  }
  @tracked mode = 'merge';
  @tracked files = [];
  ...
  get splitPreview() {
    ...
    if (this.splitHow === 'every')
      return { text: `${file.pages} separate PDFs`, many: file.pages > 1 };
    const parsed = parsePageRanges(this.splitRanges, file.pages);
    if (parsed.error) return { error: parsed.error };
    return {
      text:
        parsed.groups.length === 1
          ? `1 PDF with ${parsed.groups[0].length} pages`
          : `${parsed.groups.length} PDFs`,
      many: parsed.groups.length > 1,
    };
  }
  ...
  get canRun() {
    ...
    if (this.isMerge)
      return (
        this.files.length > 0 &&
        this.files.every((f) => f.pages && !f.rangeError)
      );
  ...
  setRange = (id, e) => {
    ...
    this.update(id, {
      range,
      rangeError: file.pages
        ? (parsePageRanges(range, file.pages).error ?? null)
        : null,
    });
    this.clearResult();
  };
  ...
  async merge() {
    ...
    const name =
      this.files.length === 1
        ? this.files[0].name.replace(/\.pdf$/i, '-pages.pdf')
        : 'merged.pdf';
  ...
  async split() {
    ...
    const groups =
      this.splitHow === 'every'
        ...
        : parsePageRanges(this.splitRanges, count).groups;
    ...
    const zip = zipSync(
      Object.fromEntries(parts.map((p) => [p.name, p.bytes])),
      { level: 0 },
    );
    return {
  ...
  <template>
    <ToolPage
      ...
      @subtitle="Merge PDFs, pick just the pages you need, or split one into several files. Your files never leave your device."
    >
      <div class="fs" {{acceptPastedFiles this.pasteFiles}}>
        <div class="fs-frame fc-panel pop-in">
          <div class="math-tabs" role="group" aria-label="Mode">
            <button
              type="button"
              class="qr-tab {{if this.isMerge 'active'}}"
              {{on "click" (fn this.setMode "merge")}}
            >Merge</button>
            <button
              type="button"
              class="qr-tab {{unless this.isMerge 'active'}}"
              {{on "click" (fn this.setMode "split")}}
            >Split</button>
          </div>
          <label
          ...
          >
            ...
            <span>{{if
                this.isMerge
                "Drop PDFs to combine, or click to browse"
                "Drop a PDF to split, or click to browse"
              }}</span>
            <input
              type="file"
              accept="application/pdf,.pdf"
              multiple={{this.isMerge}}
              class="sr-only"
        ...
        {{#if this.files.length}}
          <div class="fs-frame fc-panel pop-in">
              ...
              {{#each this.files key="id" as |f index|}}
                <li class="fs-row fc-row">
                  ...
                  <div class="fs-row-info">
                    ...
                    <span class="fs-row-size">{{formatBytes f.size}}
                      ·
                      {{if f.pages (concatPages f.pages) "reading…"}}</span>
                    {{#if f.loadError}}<span
                        class="tool-error"
                      >{{f.loadError}}</span>{{/if}}
                    {{#if f.rangeError}}<span
                        class="tool-error"
                      >{{f.rangeError}}</span>{{/if}}
                  </div>
                  {{#if this.isMerge}}
                    <div class="fs-row-status">
                      <input
                        type="text"
                        class="math-input pdf-range"
                        placeholder="All pages"
                        aria-label="Pages to include from {{f.name}}"
                        value={{f.range}}
                        {{on "input" (fn this.setRange f.id)}}
                      />
                      <button
                        type="button"
                        class="btn"
            ...
            {{#if this.isMerge}}
              <p class="tool-hint">Pages are added in list order. Leave a range
                empty for every page, or type something like 1-3, 5, 9-.</p>
            {{else}}
              <div class="math-tabs" role="group" aria-label="How to split">
                <button
                  type="button"
                  class="qr-tab {{if (eq this.splitHow 'every') 'active'}}"
                  {{on "click" (fn this.setSplitHow "every")}}
                >Every page</button>
                <button
                  type="button"
                  class="qr-tab {{if (eq this.splitHow 'ranges') 'active'}}"
                  {{on "click" (fn this.setSplitHow "ranges")}}
                >By ranges</button>
              </div>
              {{#if (eq this.splitHow "ranges")}}
                <label class="math-field">
                  <span class="qr-label is-muted">Ranges (each becomes its own
                    PDF)</span>
                  <input
                    type="text"
                    class="math-input"
                    value={{this.splitRanges}}
                    {{on "input" this.setSplitRanges}}
                  />
                </label>
              {{/if}}
              ...
              {{else if this.splitPreview}}
                <p class="tool-hint">Makes
                  {{this.splitPreview.text}}{{if
                    this.splitPreview.many
                    ", bundled in a ZIP"
                  }}.</p>
              {{/if}}
            {{/if}}
            ...
            <div class="settings-actions">
              <button
                type="button"
                class="btn active"
                disabled={{if this.canRun false true}}
                {{on "click" this.run}}
              >{{if
                  this.busy
                  "Working…"
                  (if this.isMerge "Merge PDFs" "Split PDF")
                }}</button>
              {{#if this.result}}
                <a
                  class="btn fs-save"
                ...
                ><Icon @name="download" @size={{13}} />
                  Save
                  {{this.result.name}}
                  ({{formatBytes this.result.size}})</a>
                <PrintButton
                  @url={{this.result.url}}
