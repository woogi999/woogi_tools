...
import { HAIR_STYLES, EYES, BROWS, NOSES, MOUTHS, FACE_SLIDERS, TOPS, PATTERNS, BOTTOMS, SHOES, HATS, EYEWEAR, MASKS, NECKWEAR, BACKS, PIERCINGS, MARKS, SKIN_TONES, HAIR_COLORS, EYE_COLORS, SHIRT_COLORS, ACCENT_COLORS, PANTS_COLORS, SHOE_COLORS, HAT_COLORS, randomAvatar, playerAvatar, avatarKey } from '../utils/avatar';
import { JOINTS, EXPRESSIONS, POSE_PRESETS, DEFAULT_POSE, normalisePose, poseKey, setPoseAngle, mirrorPose, randomPose } from '../utils/pose';
...
const BACKGROUND_SWATCHES = ['#F7F5EF', '#FFFFFF', '#141414', '#FFD166', '#7BDFF2', '#B388EB', '#FF8FAB', '#95D5B2'];
...
export default class AvatarEditor extends Component {
  ...
  get unsaved() {
    ...
    return Boolean(save) && (avatarKey(save.avatar) !== avatarKey(this.avatar) || poseKey(save.pose) !== poseKey(this.pose));
  }

  get jointInfo() {
    const joint = JOINTS.find((j) => j.id === this.joint) ?? JOINTS[0];
    const values = this.pose[joint.id];
    return { ...joint, axes: joint.axes.map((a) => ({ ...a, value: values[a.key] })) };
  }
  ...
  setAngle = (axis, event) => this.setPose(setPoseAngle(this.pose, this.joint, axis.key, Number(event.target.value)));
  nudgeAngle = (axis, step) => this.setPose(setPoseAngle(this.pose, this.joint, axis.key, axis.value + step));
  resetJoint = () => this.setPose({ ...this.pose, [this.joint]: DEFAULT_POSE[this.joint] });
  ...
  snapshot() {
    const canvas = this.previewHandle?.snapshot({ width: this.photoSize, height: this.framing === 'full' ? Math.round((this.photoSize * 9) / 16) : this.photoSize, frame: this.framing, background: this.transparent ? null : this.background });
    return canvas;
  }
  ...
  takePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      const name = `${(this.args.name || 'avatar').trim().replace(/[^\w-]+/g, '-').toLowerCase() || 'avatar'}-${Date.now().toString(36)}.png`;
      const link = Object.assign(document.createElement('a'), { href: this.photoUrl, download: name });
      link.click();
  ...
  saveLook = () => {
    ...
    const save = { id: Date.now().toString(36), label, avatar: this.avatar, pose: this.pose };
  ...
  updateLook = () => {
    ...
    if (!current) return;
    this.saves = this.saves.map((s) => (s.id === current.id ? { ...s, avatar: this.avatar, pose: this.pose } : s));
    storeSaves(this.saves);
    sfx('ui.confirm');
  ...
  wear = (save) => {
    if (this.args.onWear) this.args.onWear({ avatar: save.avatar, pose: save.pose, look: save.id });
    else {
  ...
  forget = async (save) => {
    if (!(await askConfirm({ title: `Delete “${save.label}”?`, message: 'This saved avatar will be gone from your profile and lobby pickers.', confirmLabel: 'Hold to delete', holdMs: 1500 }))) return;
  ...
  <template>
    <div class="avatar-editor is-tool">
      <div class="avatar-stage" {{this.follow this.avatar this.pose this.posing this.joint}}>
        {{#if this.noWebGL}}
          <AvatarPortrait @avatar={{this.avatar}} @size={{150}} />
        {{else}}
          <canvas class="avatar-canvas {{if this.posing 'is-posing'}}" aria-label={{if this.posing "Your avatar. Drag a joint handle to pose it, or drag the background to turn it." "Your avatar. Drag to turn it around."}} {{this.preview}}></canvas>
        {{/if}}
        {{#unless this.noWebGL}}
          <div class="avatar-view-buttons" role="group" aria-label="View">
            <button type="button" class="btn" {{on "click" this.viewFront}}>Front</button>
            <button type="button" class="btn" {{on "click" this.viewSide}}>Side</button>
            <button type="button" class="btn" {{on "click" this.viewBack}}>Back</button>
          </div>
        {{/unless}}
        <div class="avatar-stage-actions">
          ...
          {{#if this.unsaved}}
            <button type="button" class="btn active" {{on "click" this.updateLook}}><Icon @name="save" @size={{13}} /> Update “{{this.currentSave.label}}”</button>
          {{/if}}
          <button type="button" class="btn" disabled={{if this.canSave false true}} {{on "click" this.saveLook}}><Icon @name="star" @size={{13}} /> Save as new look</button>
        </div>
      </div>
          ...
          <div class="avatar-option-row">
            ...
            <div class="avatar-chips" role="group" aria-label="Pose presets">
              {{#each this.posePresets as |p|}}
                <button type="button" class="home-chip {{if (eq this.activePreset p.id) 'active'}}" aria-pressed={{if (eq this.activePreset p.id) "true" "false"}} {{on "click" (fn this.pickPreset p)}}>{{p.label}}</button>
              {{/each}}
            </div>
          </div>
          <OptionRow @label="Expression" @options={{this.expressions}} @value={{this.pose.expression}} @onPick={{this.pickExpression}} />
          <div class="avatar-option-row">
            <span class="qr-label is-muted">Joint <span class="tool-hint">(or click a dot on the avatar and drag it)</span></span>
            <div class="avatar-chips" role="group" aria-label="Joint to pose">
              {{#each this.joints as |j|}}
                <button type="button" class="home-chip {{if (eq this.joint j.id) 'active'}}" aria-pressed={{if (eq this.joint j.id) "true" "false"}} {{on "click" (fn this.pickJoint j.id)}}>{{j.label}}</button>
              {{/each}}
            </div>
          </div>
          <div class="avatar-sliders pose-sliders" aria-label="{{this.jointInfo.label}} angles">
            {{#each this.jointInfo.axes key="key" as |axis|}}
              <div class="avatar-slider">
                <span class="avatar-slider-label">{{axis.label}}</span>
                <button type="button" class="qr-icon-btn" aria-label="{{axis.label}}: less" {{on "click" (fn this.nudgeAngle axis -5)}}><Icon @name="minus" @size={{12}} /></button>
                <input type="range" min={{axis.min}} max={{axis.max}} step="1" value={{axis.value}} aria-label="{{this.jointInfo.label}} {{axis.label}}" {{on "input" (fn this.setAngle axis)}} />
                <button type="button" class="qr-icon-btn" aria-label="{{axis.label}}: more" {{on "click" (fn this.nudgeAngle axis 5)}}><Icon @name="plus" @size={{12}} /></button>
                <span class="avatar-slider-value">{{axis.value}}°</span>
              </div>
            {{/each}}
          </div>
          <div class="avatar-stage-actions">
            <button type="button" class="btn" {{on "click" this.resetJoint}}><Icon @name="rotate-ccw" @size={{13}} /> Reset {{this.jointInfo.label}}</button>
            <button type="button" class="btn" {{on "click" this.mirror}}><Icon @name="arrow-right-left" @size={{13}} /> Mirror</button>
            <button type="button" class="btn" {{on "click" this.randomise}}><Icon @name="dices" @size={{13}} /> Random</button>
            <button type="button" class="btn" {{on "click" this.resetPose}}><Icon @name="person-standing" @size={{13}} /> Reset pose</button>
          </div>
          <p class="tool-hint">Strike a pose! It shows on your portrait wherever your avatar appears. Save the look to keep it.</p>
        <h4 class="avatar-section-title"><Icon @name="camera" @size={{14}} /> Photo</h4>
            ...
            <div class="avatar-option-row">
              ...
              <div class="math-tabs" role="group" aria-label="Framing">
                {{#each this.framings as |f|}}
                  <button type="button" class="qr-tab {{if (eq this.framing f.id) 'active'}}" aria-pressed={{if (eq this.framing f.id) "true" "false"}} {{on "click" (fn this.setFraming f.id)}}>{{f.label}}</button>
                {{/each}}
              </div>
            </div>
            <div class="avatar-option-row">
              ...
              <div class="math-tabs" role="group" aria-label="Image size">
                {{#each this.photoSizes as |s|}}
                  <button type="button" class="qr-tab {{if (eq this.photoSize s.id) 'active'}}" aria-pressed={{if (eq this.photoSize s.id) "true" "false"}} {{on "click" (fn this.setPhotoSize s.id)}}>{{s.label}}</button>
                {{/each}}
              </div>
            </div>
            <label class="qr-switch">
              <input type="checkbox" role="switch" checked={{this.transparent}} aria-checked={{if this.transparent "true" "false"}} {{on "change" this.setTransparent}} />
              <span class="qr-switch-track" aria-hidden="true"></span>
              Transparent background
            </label>
            <div class="avatar-color-row">
              <span class="qr-label is-muted">Background</span>
              <div class="avatar-swatches" role="group" aria-label="Background colours">
                {{#each this.backgroundSwatches as |color|}}
                  <button type="button" class="avatar-swatch {{if (bgActive this.transparent this.background color) 'active'}}" style={{swatch color}} aria-label={{color}} {{on "click" (fn this.setBackground color)}}></button>
                {{/each}}
              </div>
              <ColourField @value={{this.background}} @label="Background colour" @onChange={{this.setBackground}} />
            </div>
            <p class="tool-hint">Turn the avatar to the angle you like first; the photo uses the view as it is now, with the pose you set.</p>
            <div class="avatar-stage-actions">
              <button type="button" class="btn active" disabled={{this.noWebGL}} {{on "click" this.takePhoto}}><Icon @name="camera" @size={{13}} /> Download PNG</button>
              <button type="button" class="btn" disabled={{this.noWebGL}} {{on "click" this.copyPhoto}}><Icon @name="copy" @size={{13}} /> Copy image</button>
            </div>
            {{#if this.photoNote}}<p class="tool-hint" role="status">{{this.photoNote}}</p>{{/if}}
            {{#if this.photoUrl}}<img class="avatar-photo-preview" src={{this.photoUrl}} alt="Your last avatar photo" />{{/if}}
          </div>
        {{else}}
              ...
              {{#each this.saves key="id" as |save|}}
                <li class="avatar-save {{if (eq save.id @lookId) 'is-worn'}}">
                  <button type="button" class="avatar-save-wear" aria-label="Wear {{save.label}}" title="Wear this look" {{on "click" (fn this.wear save)}}>
                    <AvatarPortrait @avatar={{save.avatar}} @pose={{save.pose}} @size={{56}} />
                  </button>
          ...
          {{else}}
            <p class="tool-hint">Nothing saved yet. Hit Save as new look to keep this one. Make as many characters as you like and swap between them in Settings or any lobby.</p>
          {{/if}}
          <p class="tool-hint">Up to {{this.maxSaves}} characters, poses and all, kept in this browser.</p>
        {{/if}}
      </div>
    </div>
  </template>
