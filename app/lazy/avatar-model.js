...
export function outlineMaterial(thickness) {
  return new ShaderMaterial({
    ...
    vertexShader:
      'uniform float thickness; void main() { vec4 p = modelViewMatrix * vec4(position, 1.0); vec3 n = normalize(normalMatrix * normal); p.xyz += n * thickness; gl_Position = projectionMatrix * p; }',
    fragmentShader:
...
export function disposeGroup(group) {
  group.traverse((obj) => {
    if (
      ...
      (obj.material?.isMeshBasicMaterial || obj.material?.isSpriteMaterial) &&
      !obj.userData.sharedMaterial
    ) {
    ...
    for (const material of obj.userData.moods?.values() ?? [])
      material.dispose();
  });
}
...
function faceTexture(kit, avatar, blink) {
  return kit.texture(`face:${avatarKey(avatar)}:${blink}`, 256, 256, (ctx, w) =>
    drawFace(ctx, avatar, w, { blink }),
  );
}

const darker = (hex, amount = 0.72) =>
  `#${new Color(hex).multiplyScalar(amount).getHexString()}`;
...
function headNormal(x, y, z, grow, out = new Vector3()) {
  ...
  return out
    .set(g(x, HEAD.x + grow), g(y, HEAD.y + grow), g(z, HEAD.z + grow))
    .normalize();
}
...
function resample(points, steps) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    out.push(
      new Vector2(
        MathUtils.lerp(r0, r1, MathUtils.clamp((y - y0) / (y1 - y0), 0, 1)),
        y,
      ),
    );
  }
...
function lathe(
  ...
  const profile = resample(points, steps).map(
...
function patternTexture(kit, a) {
  const texture = kit.texture(
    `pattern:${a.pattern}:${a.shirt}:${a.accent}`,
    256,
    128,
    (ctx, w, h) => {
...
function buildHuman(avatar, kit, group) {
  ...
  const printed =
    avatar.pattern === 'plain'
      ? shirt
      : kit.toon('#FFFFFF', { map: patternTexture(kit, avatar) });
  ...
  const add = (
    ...
    material,
    parent,
    { position, rotation, scale, thin = true, outline = true } = {},
  ) => {
  ...
  const sphere = (r, w = 12, h = 8) =>
    kit.geometry(`sphere:${r}:${w}`, () => new SphereGeometry(r, w, h));
  ...
  for (const side of [-1, 1]) {
    ...
    add(
      kit.geometry('leg', () => new CylinderGeometry(0.066, 0.06, 0.26, 12)),
      trousers ? pants : skin,
      hip,
      { position: at(0.19) },
    );
    if (bottom === 'shorts' && !dress)
      add(
        kit.geometry(
          'shorts-leg',
          () => new CylinderGeometry(0.09, 0.085, 0.1, 12),
        ),
        pants,
        hip,
        { position: at(0.27) },
    ...
    switch (avatar.shoes) {
      case 'boots':
        add(
          kit.geometry(
            'boot',
            () => new CylinderGeometry(0.08, 0.086, 0.15, 12),
          ),
          shoes,
          hip,
          { position: at(0.1) },
        );
        add(sphere(0.1), shoes, hip, {
          position: at(0.045, 0.04),
          scale: [0.85, 0.5, 1.2],
        });
        break;
      case 'flats':
        add(sphere(0.1), shoes, hip, {
          position: at(0.035, 0.03),
          scale: [0.72, 0.38, 1.12],
        });
        break;
      default:
        add(sphere(0.1), shoes, hip, {
          position: at(0.055, 0.03),
          scale: [0.8, 0.55, 1.2],
        });
        add(
          kit.geometry(
            'sole',
            () => new CylinderGeometry(0.085, 0.085, 0.025, 14),
          ),
          white,
          hip,
          { position: at(0.018, 0.035), scale: [0.95, 1, 1.4] },
        );
    }
  }
  if (bottom === 'skirt' && !dress)
    add(
      kit.geometry('skirt', () => lathe(SKIRT, { steps: 6 })),
      pants,
      group,
      { scale: [1, 1, 0.84] },
    );
  ...
  const body = add(
    kit.geometry(dress ? 'dress' : 'torso', () =>
      lathe(dress ? DRESS : TORSO, { steps: 16 }),
    ),
    innerColour,
    group,
    { scale: bodyScale, thin: false },
  );

  switch (top) {
    case 'hoodie':
      add(
        kit.geometry('hood', () => new TorusGeometry(0.14, 0.065, 8, 18)),
        shirt,
        group,
        {
          position: [0, 0.73, -0.1],
      ...
      add(
        kit.geometry(
          'pocket',
          () => new RoundedBoxGeometry(0.26, 0.1, 0.04, 2, 0.015),
        ),
        shirt,
        group,
        { position: [0, 0.4, 0.185] },
      );
      for (const side of [-1, 1])
        add(
          kit.geometry(
            'string',
            () => new CylinderGeometry(0.012, 0.012, 0.13, 6),
          ),
          accent,
          group,
          { position: [side * 0.05, 0.62, 0.165] },
        );
      break;
    case 'sweater':
      add(
        kit.geometry(
          'turtleneck',
          () => new CylinderGeometry(0.12, 0.13, 0.09, 16),
        ),
        shirt,
        group,
        { position: [0, 0.755, 0] },
      ...
      add(
        kit.geometry('jacket', () =>
          lathe(TORSO, {
            ...
            caps: false,
            phiStart: 0.34,
            phiLength: Math.PI * 2 - 0.68,
            steps: 16,
          }),
        ),
        kit.toon(avatar.shirt, { double: true }),
        group,
        { scale: bodyScale, thin: false },
      );
      break;
      ...
      add(
        kit.geometry('sailor-collar', () =>
          lathe(
            ...
            { scale: 1.06, caps: false, steps: 5 },
          ),
        ),
        kit.toon(avatar.accent, { double: true }),
        group,
        { scale: bodyScale },
      );
      for (const side of [-1, 1])
        add(
          kit.geometry('bow-wing', () => new ConeGeometry(0.05, 0.1, 4)),
          red,
          group,
          {
            position: [side * 0.05, 0.62, 0.17],
    ...
    case 'overalls':
      add(
        kit.geometry('overall-band', () =>
          lathe(
            ...
            { scale: 1.03, caps: false, steps: 4 },
          ),
        ),
        kit.toon(avatar.pants, { double: true }),
        group,
        { scale: bodyScale, thin: false },
      );
      add(
        kit.geometry(
          'bib',
          () => new RoundedBoxGeometry(0.22, 0.17, 0.035, 2, 0.012),
        ),
        pants,
        group,
      ...
      for (const side of [-1, 1]) {
        add(
          kit.geometry(
            'strap',
            () => new RoundedBoxGeometry(0.04, 0.2, 0.02, 1, 0.006),
          ),
          pants,
          group,
          { position: [side * 0.08, 0.62, 0.155], rotation: [-0.35, 0, 0] },
        );
        add(sphere(0.018, 8, 6), gold, group, {
          position: [side * 0.08, 0.54, 0.2],
          outline: false,
        });
      }
      break;
  ...
  switch (avatar.neck) {
    case 'scarf':
      add(
        kit.geometry('scarf', () => new TorusGeometry(0.15, 0.06, 8, 20)),
        accent,
        group,
        {
          position: [0, 0.74, 0],
      ...
      add(
        kit.geometry(
          'scarf-tail',
          () => new CapsuleGeometry(0.045, 0.16, 4, 8),
        ),
        accent,
        group,
        { position: [0.09, 0.6, 0.17], rotation: [0.2, 0, 0.15] },
      );
      break;
    case 'tie':
      add(
        kit.geometry('tie', () => new ConeGeometry(0.055, 0.24, 4)),
        accent,
        group,
        {
          position: [0, 0.6, 0.172],
          rotation: [Math.PI + 0.1, Math.PI / 4, 0],
          scale: [1, 1, 0.35],
        },
      );
      break;
    ...
    case 'bell':
      add(
        kit.geometry('choker', () => new TorusGeometry(0.15, 0.024, 6, 20)),
        avatar.neck === 'bell' ? red : dark,
        group,
        {
          position: [0, 0.712, 0],
          rotation: [Math.PI / 2, 0, 0],
      ...
      if (avatar.neck === 'bell')
        add(sphere(0.045), gold, group, { position: [0, 0.655, 0.155] });
      break;
    case 'necklace':
      add(
        kit.geometry('necklace', () => new TorusGeometry(0.15, 0.01, 6, 24)),
        gold,
        group,
        {
          position: [0, 0.67, 0.03],
      ...
      add(
        kit.geometry('gem', () => new ConeGeometry(0.04, 0.08, 4)),
        accent,
        group,
        { position: [0, 0.58, 0.18], rotation: [Math.PI, 0, 0] },
      );
      break;
    case 'bandana':
      add(
        kit.geometry('bandana', () => new ConeGeometry(0.15, 0.2, 3)),
        accent,
        group,
        {
  ...
  switch (avatar.back) {
    case 'backpack':
      add(
        kit.geometry(
          'backpack',
          () => new RoundedBoxGeometry(0.3, 0.32, 0.14, 3, 0.05),
        ),
        accent,
        group,
        { position: [0, 0.5, -0.24], thin: false },
      );
      add(
        kit.geometry(
          'backpack-pocket',
          () => new RoundedBoxGeometry(0.2, 0.1, 0.05, 2, 0.02),
        ),
        accent,
        group,
      ...
      for (const side of [-1, 1])
        add(
          ...
          accent,
          group,
          { position: [side * 0.1, 0.6, 0.155], rotation: [-0.35, 0, 0] },
        );
      break;
    case 'wings':
      for (const side of [-1, 1]) {
        add(sphere(0.2, 16, 10), white, group, {
          position: [side * 0.24, 0.64, -0.24],
          rotation: [0, side * 0.5, side * -0.55],
          scale: [1.15, 0.6, 0.12],
        });
        add(sphere(0.2, 16, 10), white, group, {
          position: [side * 0.2, 0.5, -0.23],
    ...
    case 'bat-wings':
      for (const side of [-1, 1])
        add(
          kit.geometry('bat-wing', () => new ConeGeometry(0.2, 0.38, 3)),
          kit.toon('#3B2A4A'),
          group,
          {
            position: [side * 0.26, 0.6, -0.2],
          ...
          () =>
            new TubeGeometry(
              new CatmullRomCurve3([
                ...
                new Vector3(0, 0.28, -0.4),
                new Vector3(0.05, 0.42, -0.52),
                new Vector3(0.12, 0.62, -0.5),
                new Vector3(0.1, 0.72, -0.42),
              ]),
              20,
              0.035,
              8,
    ...
    case 'fox-tail':
      add(
        kit.geometry('fox-tail', () => new CapsuleGeometry(0.12, 0.3, 6, 12)),
        hair,
        group,
        { position: [0, 0.42, -0.36], rotation: [-1.0, 0, 0] },
      );
      add(sphere(0.1, 12, 10), white, group, {
        position: [0, 0.56, -0.58],
        scale: [1, 1.15, 1],
      });
      break;
    default:
  ...
  for (const side of [-1, 1]) {
    ...
    group.add(shoulder);
    const arm = add(
      kit.geometry('arm', () => new CapsuleGeometry(0.05, 0.15, 4, 8)),
      longSleeves ? shirt : skin,
      shoulder,
      { position: [0, -0.1, 0] },
    );
    if (shortSleeves) {
      add(
        kit.geometry(
          'sleeve',
          () => new CylinderGeometry(0.068, 0.066, 0.09, 12),
        ),
        shirt,
        shoulder,
        { position: [0, -0.035, 0] },
      ...
      add(sphere(0.072, 12, 8), shirt, shoulder, {
        position: [0, 0.005, 0],
        scale: [1, 0.85, 1],
      });
    }
    const hand = add(sphere(0.06, 10, 8), skin, shoulder, {
      position: [0, -0.2, 0],
    });
  ...
  group.add(head);
  add(
    kit.geometry('head', () => toHead(new SphereGeometry(1, 40, 28))),
    skin,
    head,
    { thin: false },
  );
  ...
  for (const side of [-1, 1]) {
    ...
    add(sphere(0.1), skin, head, {
      position: [x, -0.05, 0.02],
      scale: [0.45, 0.8, 0.62],
    });
    if (piercings.has('ear-studs'))
      add(sphere(0.02, 8, 6), metal, head, {
        position: [x + side * 0.04, -0.11, 0.05],
        outline: false,
      });
    if (piercings.has('ear-hoops'))
      add(
        kit.geometry('hoop', () => new TorusGeometry(0.04, 0.01, 6, 16)),
        gold,
        head,
        {
          position: [x + side * 0.04, -0.15, 0.04],
          rotation: [0, Math.PI / 2, 0],
          outline: false,
        },
      );
    if (piercings.has('cartilage'))
      add(
        kit.geometry('hoop-small', () => new TorusGeometry(0.03, 0.008, 6, 14)),
        metal,
        head,
        {
          position: [x + side * 0.045, 0.03, 0],
          rotation: [0, Math.PI / 2, 0],
          outline: false,
        },
      );
  }
  const faceGeometry = kit.geometry('face', () =>
    toHead(
      new SphereGeometry(
        1,
        28,
        20,
        Math.PI / 2 - FACE_SPAN.az,
        FACE_SPAN.az * 2,
        Math.PI / 2 - FACE_SPAN.polar,
        FACE_SPAN.polar * 2,
      ),
      0.008,
    ),
  ...
  if (avatar.mask === 'fox') {
    ...
    for (const side of [-1, 1]) {
      add(
        kit.geometry('fox-ear', () => new ConeGeometry(0.06, 0.13, 4)),
        white,
        mask,
        {
          position: [side * 0.1, 0.17, 0],
          rotation: [0, 0, side * -0.3],
          scale: [1, 1, 0.5],
        },
      );
      add(sphere(0.022, 8, 6), red, mask, {
        position: [side * 0.07, 0.03, 0.08],
        scale: [1.6, 0.6, 0.5],
        outline: false,
      });
    }
    add(sphere(0.025, 8, 6), dark, mask, {
  ...
  const surface = style.none
    ? { volume: 1, front: 1, side: 1, back: 1, none: true }
    : style;
  ...
  if (!style.none) {
    ...
    if (style.under)
      add(
        kit.geometry(`hair:${style.id}:under`, () => hairGeometry(style.under)),
        kit.toon(
          `#${new Color(avatar.skin).lerp(new Color(avatar.hairColor), 0.45).getHexString()}`,
          { double: true },
        ),
        head,
      );
    add(
      kit.geometry(`hair:${style.id}`, () => hairGeometry(style)),
      hairDouble,
      head,
      { thin: false },
    );
    style.layers?.forEach((layer, i) =>
      add(
        kit.geometry(`hair:${style.id}:${i}`, () => hairGeometry(layer)),
        hairDouble,
        head,
      ),
    );
    const cone = kit.geometry(
      'hair-spike',
      () => new ConeGeometry(0.1, 0.34, 6),
    );
    if (style.crown) {
      for (let i = 0; i < 7; i++) {
        ...
        point(
          ...
          head,
          at,
          new Vector3(...at).normalize().add(new Vector3(0, 0.5, -0.35)),
          [0.9, 1.05, 0.5],
        );
      }
    }
    if (style.fin) {
      ...
      for (let i = 0; i <= 10; i++) {
        ...
        point(
          ...
          head,
          at,
          new Vector3(...at).normalize().add(new Vector3(0, 0.25, -0.3)),
          [0.28, height, 1.7],
        );
      }
    }
    if (style.quiff)
      add(sphere(0.24, 18, 12), hair, head, {
        position: [0.02, crown - 0.03, HEAD.z * 0.5],
        scale: [1.3, 0.78, 1.1],
      });
    const tie = (position, size = 1) =>
      add(sphere(0.065, 10, 8), accent, head, {
        position,
    ...
    if (style.ponytail) {
      ...
      tie(at);
      add(
        kit.geometry('tail', () => new CapsuleGeometry(0.11, 0.4, 4, 10)),
        hair,
        head,
        { position: [0, at[1] - 0.3, at[2] - 0.12], rotation: [0.35, 0, 0] },
      );
    }
    if (style.twintails) {
      for (const side of [-1, 1]) {
        ...
        add(
          kit.geometry(
            'twintail',
            () => new CapsuleGeometry(0.11, 0.48, 4, 10),
          ),
          hair,
          head,
          {
            position: [at[0] + side * 0.1, at[1] - 0.34, at[2] - 0.02],
    ...
    if (style.braid) {
      ...
      for (let i = 0; i < 5; i++)
        add(sphere(0.1, 12, 8), hair, head, {
          position: [0, at[1] - i * 0.13, at[2] - 0.02 - i * 0.02],
          scale: [1 - i * 0.08, 0.9, 0.9],
        });
      tie([0, at[1] - 0.66, at[2] - 0.12], 0.8);
    }
    if (style.bun)
      add(sphere(0.19, 14, 10), hair, head, {
        position: [0, crown - 0.04, -0.3],
      });
    if (style.buns)
      for (const side of [-1, 1])
        add(sphere(0.16, 14, 10), hair, head, {
          position: [side * 0.3, crown - 0.08, -0.02],
        });
  }
  if (avatar.ahoge && !style.none) {
    const strand = kit.geometry(
      `ahoge:${crown.toFixed(3)}`,
      () =>
        new TubeGeometry(
          new CatmullRomCurve3([
            new Vector3(0, crown - 0.04, 0.04),
            new Vector3(0.01, crown + 0.14, 0.07),
            new Vector3(0.1, crown + 0.27, 0.03),
            new Vector3(0.2, crown + 0.22, -0.03),
          ]),
          16,
          0.03,
          6,
...
function buildHat(
  ...
  const grow = style.none
    ? 0
    : Math.max(
        hairGrow(style, 0),
        ...(style.layers ?? []).map((layer) => hairGrow(layer, 0)),
      ) +
      (style.lift ?? 0) * 0.5;
  // A cap of the head's own shape, `down` (as a fraction of π) from the crown.
  const shell = (key, down, out) =>
    kit.geometry(`shell:${key}:${grow.toFixed(3)}`, () =>
      toHead(
        new SphereGeometry(1, 32, 12, 0, Math.PI * 2, 0, Math.PI * down),
        grow + out,
      ),
    );
  const band = (key, from, to, out) =>
    kit.geometry(`band:${key}:${grow.toFixed(3)}`, () =>
      toHead(
        new SphereGeometry(
          1,
          32,
  ...
  switch (avatar.hat) {
    case 'cap': {
      ...
      add(
        kit.geometry('brim', () => new CylinderGeometry(0.3, 0.3, 0.03, 20)),
        hatColor,
        head,
        {
          position: [0, y + 0.02, front + 0.12],
          rotation: [0.12, 0, 0],
          scale: [1, 1, 0.8],
        },
      );
      add(sphere(0.035, 8, 6), hatColor, head, {
        position: [0, crown + 0.03, 0],
      });
      break;
    }
    case 'beanie':
      ...
      add(
        band('beanie-cuff', 0.38, 0.5, 0.08),
        kit.toon(darker(avatar.hatColor, 0.8)),
        head,
      );
    ...
    case 'bucket': {
      ...
      add(
        kit.geometry(
          'bucket-brim',
          () =>
            new CylinderGeometry(HEAD.x + 0.02, HEAD.x + 0.2, 0.1, 32, 1, true),
        ),
        kit.toon(avatar.hatColor, { double: true }),
        head,
        {
          position: [0, y - 0.03, 0],
          scale: [1 + grow, 1, (HEAD.z + 0.1) / (HEAD.x + 0.1) + grow],
        },
      );
      break;
    }
    case 'pail': {
      ...
      add(
        kit.geometry(
          `pail:${bottom.toFixed(3)}:${height.toFixed(3)}`,
          () =>
            new CylinderGeometry(bottom * 0.78, bottom, height, 28, 1, true),
        ),
        kit.toon(avatar.hatColor, { double: true }),
        pail,
        { position: [0, height / 2, 0], thin: false },
      );
      add(
        kit.geometry(
          `pail-top:${bottom.toFixed(3)}`,
          () => new CircleGeometry(bottom * 0.78, 28),
        ),
        hatColor,
        pail,
        {
      ...
      add(
        kit.geometry(
          `pail-lip:${bottom.toFixed(3)}`,
          () => new TorusGeometry(bottom, 0.025, 6, 32),
        ),
        kit.toon(darker(avatar.hatColor, 0.8)),
        pail,
        { rotation: [Math.PI / 2, 0, 0] },
      ...
      add(
        kit.geometry(
          `pail-handle:${reach.toFixed(3)}`,
          () => new TorusGeometry(reach, 0.014, 6, 24, Math.PI),
        ),
        kit.toon('#B8BEC8'),
        handle,
      );
      for (const side of [-1, 1])
        add(sphere(0.035, 8, 6), kit.toon('#B8BEC8'), pail, {
          position: [side * reach, height * 0.45, 0],
        });
      break;
    }
    case 'traffic-cone': {
      ...
      add(
        kit.geometry(
          'cone-base',
          () => new RoundedBoxGeometry(0.62, 0.06, 0.62, 2, 0.02),
        ),
        kit.toon('#E0600B'),
        cone,
        { position: [0, 0.03, 0], thin: false },
      );
      add(
        kit.geometry('cone', () => new ConeGeometry(base, tall, 24)),
        orange,
        cone,
        { position: [0, 0.06 + tall / 2, 0], thin: false },
      );
      for (const [from, to] of [
        ...
        add(
          ...
          white,
          cone,
          { position: [0, 0.06 + (from + to) / 2, 0], outline: false },
        );
      }
      break;
    }
    case 'beret':
      add(sphere(0.5, 24, 12), hatColor, head, {
        position: [0.06, crown - 0.03, -0.02],
        rotation: [0, 0, -0.18],
        scale: [1.18 + grow, 0.32, 1.1 + grow],
        thin: false,
      });
      add(
        kit.geometry(
          'beret-stem',
          () => new CylinderGeometry(0.015, 0.02, 0.07, 6),
        ),
        hatColor,
        head,
        { position: [0.02, crown + 0.14, -0.02] },
      );
      break;
    case 'straw': {
      ...
      add(
        kit.geometry(
          'straw-brim',
          () => new CylinderGeometry(0.8, 0.82, 0.03, 36),
        ),
        straw,
        head,
        { position: [0, crown - 0.1, 0], thin: false },
      );
      add(
        kit.geometry(
          'straw-crown',
          () => new CylinderGeometry(0.34, 0.38, 0.2, 24),
        ),
        straw,
        head,
        { position: [0, crown + 0.0, 0] },
      );
      add(
        kit.geometry(
          'straw-band',
          () => new CylinderGeometry(0.385, 0.385, 0.06, 24, 1, true),
        ),
        kit.toon(avatar.hatColor, { double: true }),
        head,
        { position: [0, crown - 0.05, 0], outline: false },
      );
      break;
    }
    case 'tophat':
      add(
        kit.geometry(
          'tophat-brim',
          () => new CylinderGeometry(0.48, 0.48, 0.03, 32),
        ),
        hatColor,
        head,
        { position: [0, crown - 0.04, 0], thin: false },
      );
      add(
        kit.geometry(
          'tophat',
          () => new CylinderGeometry(0.29, 0.27, 0.46, 24),
        ),
        hatColor,
        head,
        { position: [0, crown + 0.2, 0], thin: false },
      );
      add(
        kit.geometry(
          'tophat-band',
          () => new CylinderGeometry(0.275, 0.275, 0.07, 24, 1, true),
        ),
        dark,
        head,
        { position: [0, crown + 0.03, 0], outline: false },
      );
      break;
    case 'witch':
      add(
        kit.geometry(
          'witch-brim',
          () => new CylinderGeometry(0.62, 0.64, 0.03, 32),
        ),
        hatColor,
        head,
        { position: [0, crown - 0.06, 0], thin: false },
      );
      add(
        kit.geometry('witch-cone', () => new ConeGeometry(0.36, 0.74, 24)),
        hatColor,
        head,
        {
          position: [0, crown + 0.3, -0.04],
          rotation: [-0.2, 0, 0.12],
          thin: false,
        },
      );
      add(
        kit.geometry(
          'witch-band',
          () => new CylinderGeometry(0.335, 0.35, 0.06, 24, 1, true),
        ),
        gold,
        head,
        {
          position: [0, crown - 0.0, -0.01],
    ...
    case 'party': {
      const tilt = -0.28;
      add(
        kit.geometry('party', () => new ConeGeometry(0.16, 0.42, 20)),
        hatColor,
        head,
        { position: [0.1, crown + 0.15, 0], rotation: [0, 0, tilt] },
      );
      add(sphere(0.055, 10, 8), white, head, {
        position: [
          0.1 - Math.sin(tilt) * 0.22,
          crown + 0.15 + Math.cos(tilt) * 0.22,
          0,
        ],
      });
      break;
    ...
    case 'frog':
      ...
      for (const side of [-1, 1]) {
        add(sphere(0.12, 14, 10), white, head, {
          position: [side * 0.2, crown + 0.05, 0.16],
        });
        add(sphere(0.05, 10, 8), dark, head, {
          position: [side * 0.2, crown + 0.07, 0.27],
          outline: false,
        });
      }
      break;
    case 'cat-ears':
      for (const side of [-1, 1]) {
        point(
          kit.geometry('cat-ear', () => new ConeGeometry(0.15, 0.3, 4)),
          hair,
          head,
          on(side * 0.62, 0.2, -0.03),
          new Vector3(side * 0.35, 1, 0),
          [1, 1, 0.45],
        );
        point(
          kit.geometry('cat-ear-inner', () => new ConeGeometry(0.08, 0.18, 4)),
          pink,
          head,
          on(side * 0.56, 0.19, 0.03),
          new Vector3(side * 0.35, 1, 0.25),
          [1, 1, 0.3],
        );
      }
      break;
    case 'bear-ears':
      for (const side of [-1, 1]) {
        ...
        add(sphere(0.13, 14, 10), hair, head, {
          position: at,
          scale: [1, 1, 0.5],
        });
        add(sphere(0.07, 10, 8), pink, head, {
          position: [at[0], at[1], at[2] + 0.05],
          scale: [1, 1, 0.3],
          outline: false,
        });
      }
    ...
    case 'bunny-ears':
      for (const side of [-1, 1]) {
        ...
        add(
          ...
          head,
          {
            position: [at[0] + side * 0.05, at[1] + 0.22, at[2]],
        ...
        add(
          kit.geometry(
            'bunny-inner',
            () => new CapsuleGeometry(0.035, 0.26, 4, 8),
          ),
          pink,
          head,
          {
            position: [at[0] + side * 0.05, at[1] + 0.22, at[2] + 0.03],
            rotation: [0, 0, side * -0.2],
            scale: [1, 1, 0.3],
            outline: false,
          },
    ...
    case 'horns':
      for (const side of [-1, 1])
        point(
          kit.geometry('horn', () => new ConeGeometry(0.07, 0.3, 8)),
          kit.toon('#E8E1CF'),
          head,
          on(side * 0.55, 0.2, -0.03),
          new Vector3(side * 0.6, 1, 0.3),
          [1, 1, 1],
        );
      break;
    case 'crown': {
      ...
      add(
        kit.geometry(
          'crown-band',
          () => new CylinderGeometry(0.2, 0.22, 0.1, 16, 1, true),
        ),
        kit.toon('#F2C230', { double: true }),
        head,
        { position: [0, crown + 0.02, 0] },
      );
      for (let i = 0; i < 5; i++) {
        const t = (i / 5) * Math.PI * 2;
        add(
          kit.geometry('crown-point', () => new ConeGeometry(0.05, 0.12, 4)),
          goldCrown,
          head,
          { position: [Math.sin(t) * 0.2, crown + 0.12, Math.cos(t) * 0.2] },
        );
      }
      break;
    }
    case 'halo': {
      const halo = new Mesh(
        kit.geometry('halo', () => new TorusGeometry(0.28, 0.035, 8, 28)),
        kit.toon('#F5C518'),
      );
    ...
    case 'headphones': {
      ...
      add(
        kit.geometry(
          `arch:${phones}`,
          () => new TorusGeometry(1, phones ? 0.04 : 0.035, 8, 32, Math.PI),
        ),
        hatColor,
        head,
        {
          position: [0, 0, phones ? 0 : 0.08],
          scale: [HEAD.x + grow + out, HEAD.y + grow + out, 1],
        },
      );
      if (phones)
        for (const side of [-1, 1])
          add(
            ...
            head,
            {
              position: [side * (HEAD.x + grow + 0.03), 0, 0],
    ...
    case 'bow': {
      ...
      for (const side of [-1, 1])
        add(
          kit.geometry('hair-bow', () => new ConeGeometry(0.1, 0.18, 4)),
          hatColor,
          head,
          {
            position: [at[0] + side * 0.09, at[1], at[2]],
            rotation: [0, 0, side * (Math.PI / 2)],
          },
        );
    ...
    case 'flower': {
      ...
      for (let i = 0; i < 5; i++) {
        const t = (i / 5) * Math.PI * 2;
        add(sphere(0.055, 10, 8), kit.toon('#FFB3C7'), head, {
          position: [
            at[0] + Math.cos(t) * 0.07,
            at[1] + Math.sin(t) * 0.07,
            at[2],
          ],
          outline: false,
        });
      }
      add(sphere(0.04, 8, 6), kit.toon('#F2C230'), head, {
        position: [at[0], at[1], at[2] + 0.03],
      });
      break;
    }
    case 'leaf':
      add(
        kit.geometry('leaf', () => new ConeGeometry(0.1, 0.28, 6)),
        kit.toon('#6DBE45'),
        head,
        {
          position: [0.04, crown + 0.1, 0.02],
          rotation: [0.2, 0, -0.5],
          scale: [1, 1, 0.25],
        },
      );
      add(
        kit.geometry(
          'leaf-stem',
          () => new CylinderGeometry(0.012, 0.012, 0.08, 5),
        ),
        kit.toon('#4E8A30'),
        head,
        { position: [-0.03, crown - 0.0, 0.0], rotation: [0, 0, -0.5] },
      );
      break;
    case 'clips':
      for (const [frac, tilt] of [
        [0.3, 0.5],
        [0.37, 0.25],
      ]) {
        ...
        add(
          kit.geometry(
            'clip',
            () => new RoundedBoxGeometry(0.12, 0.03, 0.02, 1, 0.008),
          ),
          hatColor,
          head,
          { position: at, rotation: [0, -0.5, tilt] },
        );
      }
      break;
    case 'goggles':
      ...
      for (const side of [-1, 1]) {
        ...
        const lens = point(
          kit.geometry(
            'goggle',
            () => new CylinderGeometry(0.1, 0.1, 0.06, 18),
          ),
          gold,
          head,
          at,
          new Vector3(...at).normalize(),
          [1, 1, 1],
        );
        add(
          kit.geometry('goggle-glass', () => new CircleGeometry(0.075, 18)),
          kit.toon('#7FC8E8'),
          lens,
          {
            position: [0, 0.031, 0],
...
function buildRobot(avatar, kit, group) {
  ...
  const add = (
    ...
    material,
    parent,
    { position, rotation, scale, thin = true } = {},
  ) => {
  ...
  const legGeometry = kit.geometry(
    'robot-leg',
    () => new CylinderGeometry(0.07, 0.07, 0.26, 10),
  );
  const footGeometry = kit.geometry(
    'robot-foot',
    () => new RoundedBoxGeometry(0.18, 0.08, 0.24, 2, 0.03),
  );
  for (const side of [-1, 1]) {
  ...
  const torso = add(
    kit.geometry(
      'robot-torso',
      () => new RoundedBoxGeometry(0.62, 0.55, 0.46, 3, 0.1),
    ),
    body,
    group,
    { position: [0, 0.52, 0], thin: false },
  );
  const light = new Mesh(
    kit.geometry('robot-light', () => new CircleGeometry(0.06, 16)),
    glow,
  );
  ...
  if (avatar.head === 'dome') {
    add(
      kit.geometry(
        'robot-dome',
        () => new SphereGeometry(ROBOT_HEAD_R, 28, 20),
      ),
      metal,
      head,
      { scale: [1.05, 0.92, 1], thin: false },
    );
    face = new Mesh(
      kit.geometry(
        'robot-face',
    ...
    add(
      kit.geometry(
        `robot-${avatar.head}`,
        () => new RoundedBoxGeometry(...size, 4, 0.14),
      ),
      metal,
      head,
      { thin: false },
    ...
    face = new Mesh(
      kit.geometry(
        `robot-screen-${avatar.head}`,
        () => new PlaneGeometry(size[0] * 0.94, size[1] * 1.05),
      ),
      new MeshBasicMaterial({
        map: faceOpen,
        transparent: true,
  ...
  add(
    kit.geometry(
      'robot-antenna',
      () => new CylinderGeometry(0.022, 0.022, 0.26, 6),
    ),
    dark,
    head,
  ...
  const ball = new Mesh(
    kit.geometry('robot-ball', () => new SphereGeometry(0.075, 12, 10)),
    glow,
  );
  ...
  for (const side of [-1, 1])
    add(
      kit.geometry(
        'robot-bolt',
        () => new CylinderGeometry(0.1, 0.1, 0.08, 14),
      ),
      dark,
      head,
      {
        position: [side * (halfWidth + 0.03), 0, 0],
        rotation: [0, 0, Math.PI / 2],
      },
    );

  const arms = [];
  const armGeometry = kit.geometry(
    'robot-arm',
    () => new CylinderGeometry(0.05, 0.05, 0.22, 8),
  );
  const handGeometry = kit.geometry(
    'robot-hand',
  ...
  for (const side of [-1, 1]) {
    ...
    const hand = add(handGeometry, metal, shoulder, {
      position: [0, -0.28, 0],
    });
...
export function buildAvatar(avatarInput, kit) {
  ...
  const parts =
    avatar.type === 'robot'
      ? buildRobot(avatar, kit, group)
      : buildHuman(avatar, kit, group);
  Object.assign(group.userData, parts, {
    avatar,
    moods: new Map(),
    robot: avatar.type === 'robot',
    faceOpen: parts.face.material,
    faceClosed: new MeshBasicMaterial({
      map: faceTexture(kit, avatar, 1),
      transparent: true,
      alphaTest: 0.35,
      depthWrite: false,
    }),
...
const MOODS = {
  stunned: {
    human: { eyes: 'dizzy', brows: 'worried', mouth: 'o' },
    robot: { eyes: 'x', mouth: 'flat' },
  },
  angry: {
    human: { brows: 'angry', mouth: 'frown' },
    robot: { eyes: 'x', mouth: 'flat' },
  },
  annoyed: {
    human: { eyes: 'sleepy', brows: 'angry', mouth: 'flat' },
    robot: { eyes: 'sleepy', mouth: 'flat' },
  },
  shocked: {
    human: { eyes: 'shocked', brows: 'worried', mouth: 'o' },
    robot: { eyes: 'blank', mouth: 'tiny' },
  },
  sad: {
    human: { eyes: 'sleepy', brows: 'worried', mouth: 'frown' },
    robot: { eyes: 'sleepy', mouth: 'flat' },
  },
  smug: {
    human: { eyes: 'wink', mouth: 'smirk' },
    robot: { eyes: 'happy', mouth: 'smile' },
  },
  joy: {
    human: { eyes: 'happy', brows: 'soft', mouth: 'grin' },
    robot: { eyes: 'happy', mouth: 'grin' },
  },
  clap: {
    human: { eyes: 'happy', mouth: 'smile' },
    robot: { eyes: 'happy', mouth: 'smile' },
  },
  cheer: {
    human: { eyes: 'happy', mouth: 'open' },
    robot: { eyes: 'big', mouth: 'grin' },
  },
};
...
export function moodFace(group, kit, mood) {
  ...
  if (!moods.has(mood))
    moods.set(
      mood,
      new MeshBasicMaterial({
        map: faceTexture(kit, { ...avatar, ...overrides }, 0),
        transparent: true,
        alphaTest: 0.35,
        depthWrite: false,
      }),
...
export function applyPose(
  ...
  head.rotation.set(
    pose.head.nod * DEG,
    pose.head.turn * DEG,
    (-pose.head.tilt + (idle ? Math.sin(now / 800) * 3.4 : 0)) * DEG,
  );
  ...
  [pose.armL, pose.armR].forEach((arm, i) => {
    ...
    const lift =
      Math.max(Math.max(0, arm.raise), Math.abs(arm.swing) * 0.6) / 175;
  ...
  const moodMaterial =
    pose.expression === 'neutral'
      ? faceOpen
      : moodFace(group, kit, pose.expression);
...
export function createAvatarPreview(canvas, { onPose, onSelect } = {}) {
  ...
  const shadowOuter = new MeshBasicMaterial({
    color: '#000000',
    transparent: true,
    opacity: 0.12,
    depthWrite: false,
  });
  const shadowInner = new MeshBasicMaterial({
    color: '#000000',
    transparent: true,
    opacity: 0.2,
    depthWrite: false,
  ...
  const handleActive = new MeshBasicMaterial({
    color: '#ffb020',
    depthTest: false,
    transparent: true,
    opacity: 1,
  ...
  const buildHandles = () => {
    ...
    for (const id of Object.keys(HANDLE_AT)) {
      ...
      const handle = new Mesh(
        handleGeometry,
        id === selected ? handleActive : handleMaterial,
      );
  ...
  const pickJoint = (event) => {
    ...
    const hit = raycaster
      ...
      .find((h) => h.object.isMesh && !h.object.userData.joint);
    if (!hit) return null;
    const owners = {
      head: avatar.userData.head,
      armL: avatar.userData.arms?.[0],
      armR: avatar.userData.arms?.[1],
      legL: avatar.userData.legs?.[0],
      legR: avatar.userData.legs?.[1],
    };
  ...
  const onMove = (event) => {
    ...
    if (press.joint) {
      ...
      const facing =
        Math.cos(stage.rotation.y + pose.root.turn * DEG) < 0 ? -1 : 1;
      const sideSign =
        press.joint === 'armL' || press.joint === 'legL' ? -1 : 1;
      const acrossSign =
        press.joint === 'root' || press.joint === 'head' ? 1 : sideSign;
      const next = {
        ...press.start,
        [across]: press.start[across] + dx * scale * acrossSign * facing,
        [upDown]:
          press.start[upDown] +
          (press.joint.startsWith('arm') ? -dy : dy) * scale,
      };
      if (press.joint.startsWith('leg'))
        next.swing = press.start.swing - dy * scale;
...
export const portraitKey = (avatarInput, pose, size) =>
  `${avatarKey(avatarInput)}#${pose ? poseKey(pose) : ''}@${size}`;

// Whether a portrait is already rendered, so it can be drawn without any flash.
export const hasPortrait = (avatarInput, pose, size) =>
  portraits.has(portraitKey(avatarInput, pose, size));
...
export function paintPortrait(target, avatarInput, pose = null) {
  ...
  if (!image) {
    ...
    if (portraits.size > MAX_PORTRAITS)
